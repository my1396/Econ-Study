...
- **<span class="env-green">A space</span> should be placed between the numerical value and the unit symbol.** 
  ...
  <span class="env-green">**Exceptions**</span> for percentage and temperatures. 10% and 17ºC. They do <span class="env-green">NOT</span> need a space between numbers and units.
...
- **Physical units must <span class="env-green">not</span> be italicized.** For example, 1.2 dB is correct, while 1.2 *dB* is not.

- **Mathematical constants** whose values never change should be **upright**. 
  
  For instance,
  
  - $\uppi$ (`\uppi` supported by `library(upgreek)`, upright Greeks) , vs. $\pi$ (`\pi`, default, italicized),
  - log base $\mathrm e$, and the imaginary unit $\mathrm i^2=-1$, 
  
  are recommended to be NOT in italic SO 80000, IUPAC, NIST, and ACS, but italicizing these expressions is <u>also very common</u>.  

- The **generic function symbols**, such as $f(x)$ and $g(x)$, should be <span class="env-green">*italicized*</span>. 

- The **standard function names** should be in **upright**, such as 
  
  - Trig func: $\sin \theta$ and $\cos \theta$, note the space between the function name and the variable, which is automatically generated by the `\sin` and `\cos` commands.
  - log func: $\ln x$ (`\ln x`) and $\log x$ (`\log x`),
  - Difference operator: $\Updelta$ (`\Updelta`, need to load `library(upgreek)`) vs $\Delta$ (doesn't seem to have much difference)
  - Small difference: $\updelta$ (`\updelta`) vs $\delta$ (lowercase difference is more obvious)
  - Infinitesimal difference: $\mathrm{d}$ (`\mathrm{d}`) vs $d$

  <span class="env-green">`library(upgreek)`</span> provides upright Greek letters. Basic syntax is `\up<greek letter>` for lowercase and `\Up<greek letter>` for uppercase Greeks.
  
  <hr>
  
  The general rule is <span class="env-green">*italic*</span> vs. <span style='color:#FF9900'>upright</span>:

  - Physical <span class="env-green">quantities or variables</span> are in <span class="env-green">*italic*</span>

    <span class="env-green">Vectors, tensors, and matrices</span> are usually denoted using a bold-face (heavy) font, but they should still be italic since they are still quantities.
...
- Division
  ...
  $x/y$ note there is no space before and after the slash `/`.
...
- Vectors should be in **bold** type.
  ...
  If you use comma to separate elements, the vector will look very crowded as follows. Besides, comma often indicates row breaks. 
  So it is NOT recommended to use comma to separate element in a row vector as it might look cluttered as follows. ❌
...
- $\boldsymbol{x} = (x_1, x_2, x_3)$ refers to a column vector, as the <span class="env-green">comma indicates row breaks</span>.

  $\boldsymbol{x} = \begin{pmatrix}x_1 & x_2 & x_3\end{pmatrix}$ refers to one row.

  ❗Caution: This convention is NOT strictly followed! You have to <span class="env-green">use discretion to determine based on the context</span>!

- Matrices

  Assume $\bX$ is a $n\times K$ matrix

  - Written as row vectors $\bx_i,$ $i=1,\cdots,n$:
  ...
  - Written as column vectors $\bx_j,$ $j=1,\cdots,K$:

    $$
    \bX = \begin{bmatrix}
    \bx_1, \bx_2, \cdots, \bx_K
    \end{bmatrix}
    $$

  See <a href="{{site.baseurl}}/2023/07/04/Econ-Notations.html">HERE</a> for more conventions for matrix, vectors.

- There should be a space before and after arithmetic operators. E.g., 2.5 ± 0.07. 

- Number range: Use <span class="env-green">two hypens</span> or opt + hyphen (<span class="env-green">an en dash</span>) and do not include a space either before or after the dash.

  Read chapters 7–9 before Friday.

  It must have been 1–2 degrees yesterday.

  - Do <span class="env-red">NOT</span> mix en dash with ‘between/and’ and ‘from/to'. ❌
...
- When to spell out numbers?
  ...
  - <span class="env-green">When the number is less than 10, they should be spelled out;</span> 10 以下的数字要拼写。
  - Use Arabic numerals for 10 and above; 10 以及 10 以上的数字用阿拉伯数字表示。
  - <span class="env-green">When numbers appear in the beginning of a sentence.</span> 以数字开头的句子，数字要拼写出来。

- Decades are written in numerals without an apostrophe: 1970s. 年份都是用numerals.

- Currency format

  - All currency symbols are placed before the number amount <span class="env-green">without any spacing</span>. 

    E.g, $500.

  - The abbreviation of a currency (e.g. USD or, EUR) should precede the number amount <span class="env-green">with a space</span>.
...
- Variables should always be set in ***italic*** in both text and in equations.
  ...
  - Identifiers (which can be **superscript or subscript** words, abbreviations, constants, or numbers) are not italicized. E.g., $M_{\text{girls}}=4.22,$ the symbol for mean is italicized, but the nonvariable identifier `girls` is NOT italicized.
  
  - If the superscript or subscript is a variable, denoting time or unit, it should be in italic. E.g., $y_t$ denoting $y$ at time $t$; $x_i$ denoting $x$ for individual $i$.

- Functions should be as roman type. E.g., $\exp, \ln, \arg$.

- If you need to break <u>one long expression</u>, the <span class="env-green">math operator such as `+` should be on the next line</span> and the continuing lines shall have additional indentation.
...
- Sometimes you use <span class="env-green">different fonts</span> for math-mode numerals and text-mode numerals. This is when you want to put all your numbers inside dollar signs `$...$`.
...
- When you have a negative number, you need to type as a formula as `-` outside math mode prints a hypen.
  ...
  Excerpt source: Donald E. Knuth 1989, TUGboat -- ["Typesetting Concrete Mathematics"](http://tug.org/TUGboat/tb10-1/tb23knut.pdf).
...
- **En dash (–)** The champion of the hour. This is the little guy – or gal – you need for **number ranges**. So called because it’s the length of a lower-case ‘n’. 
  ...
  - How you type an en dash:
    ...
    - Use <span class="env-green">`\textendash`</span> safe to encoding changes, ✅
...
- **Em dash (—)** The longest of the three dash types. It has its specific uses, but a number range is not one of them. And it’s the length of a lower-case ‘m’.
  ...
  - How you type em dash: 
    ...
    - Use <span class="env-green">`\textemdash`</span> safe to encoding changes, ✅
...
- Table labels, titles, and captions are placed on <span class="env-green">top</span> of the table.
...
- Figure titles and captions go <span class="env-green">below</span> the figure.
...
- Ensure that each bullet item is a full sentence.  每个点是句子的时候要<span class="env-green">大写首字母</span>，且以<span class="env-green">句号结尾</span>。
...
- start each bullet point with a lowercase letter  每个点不是完整句子的时候，<span class="env-green">首字母小写</span>，<span class="env-green">结尾不加标点</span>
...
1. **Phrases used within a sentence** 
   ...
   <table><tbody>
     <tr>
       <td>I can play quite a few musical instruments, <span class="env-red">for example</span>, the flute, the guitar, and the piano. </td>
     </tr>
     <tr>
       <td>I can play quite a few musical instruments, <span class="env-red">for instance,</span> the flute, the guitar, and the piano.</td>
     </tr>
     <tr>
       <td>I can play quite a few musical instruments, <span class="env-red">e.g.,</span> the flute, the guitar, and the piano.</td>
     </tr>
     <tr>
       <td>I can play quite a few musical instruments, <span class="env-red">such as</span> the flute, the guitar, and the piano.</td>
     </tr>
   </tbody>
   </table>
   ...
   **Such as** does NOT have a comma after it; the first three phrases have commas before and after them.
   ...
   <table><tbody>
     <tr>
       <td>Car companies <span class="env-red">such as</span> Toyota and Ford manufacture their automobiles in many different countries around the world. </td>
     </tr>
   </tbody>
   </table>
...
2. **Phrases used to begin a new sentence**
   ...
   <table><tbody>
     <tr>
       <td> My father loves going to restaurants which serve exotic foods.  <span class="env-red">For example,</span> last week he went to a restaurant which serves deep-fried rattlesnake.  </td>
     </tr>
       <tr><td>My father loves going to restaurants which serve exotic foods. <span class="env-red">For instance,</span> last week he went to a restaurant which serves deep-fried rattlesnake. </td>
     </tr>
   </tbody>
   </table>
...
- Use <span class="env-green">a space both before and after</span> an ellipsis. 
...
- When to use double quotation marks:
  ...
  - <u>First introducing a word or phrase <span class="env-green">ironically</span></u>, as slang, or newly coined: I was not what you would call a "normal" teenager. The study produced what I will refer to as "alpha-particles."
...
- <span class="env-green">Typesetting of double quotation marks</span>:
  ...
  - Place commas (`,`), periods (`.`), exclamation marks (`!`), and question marks (`?`) <u>inside quotation marks</u>; all other punctuation marks are placed <u>outside quotation marks.</u>
    ...
    | ------------------------------------------------------------ | ---------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
    | Comma                                                        | Inside                                               | Many dream images were characterized as <span class="env-green">“</span>raw<span class="env-green">,” “</span>powerful<span class="env-green">,”</span> and <span class="env-green">“</span>evocative<span class="env-green">.”</span> |                                                              |
    | Period                                                       | Inside                                               | Participants who kept dream diaries described themselves as <span class="env-green">“</span>introspective<span class="env-green">”</span> and <span class="env-green">“</span>thoughtful<span class="env-green">.”</span> |                                                              |
    | Question mark or exclamation point (<span class="env-green">part of quoted material</span>) | Inside                                               | The Dream Questionnaire items included <span class="env-green">“</span>How often do you remember your dreams<span class="env-green">?”</span> and <span class="env-green">“</span>What do you most often dream about<span class="env-green">?”</span> We found intriguing results. | When a quotation ending in a question mark or exclamation point ends a sentence, no extra period is needed. |
    | Quotation within a quotation + period or comma               | Inside                                               | Some participants were skeptical about the process<span class="env-green">: “</span>I don’t put any stock in these <span class="env-green">‘</span>dream diaries<span class="env-green">.’”</span> | When multiple quotation marks are used for quotations within quotations, keep the quotation marks together (put periods and commas inside both; put semi-colons, colons, etc., outside both). |
    |                                                              |                                                      |                                                              |                                                              |
    | Parentheses                                                  | Outside                                              | Barris (2010) argued that <span class="env-green">“</span>dreams express and work with the logic of gaining a sense of and a relation to ourselves, our lives, or our sense of reality as a whole<span class="env-green">” (p. 4).</span> | See more examples of how to cite direct quotations [here](https://blog.apastyle.org/apastyle/2010/03/how-to-cite-direct-quotations.html). |
    | Semi-colon                                                   | Outside                                              | At the beginning of the study, participants described their dream recall rate as <span class="env-green">“</span>low to moderate<span class="env-green">”;</span> at the end, they described it as “moderate to high<span class="env-green">.”</span> |                                                              |
    | Colon                                                        | Outside                                              | Participants stated they were <span class="env-green">“</span>excited to begin<span class="env-green">”:</span> We controlled for participants' expectations in our study. |                                                              |
    | Question mark or exclamation point (<span class="env-green">**NOT** part of quoted material</span>) | Outside                                              | How will this study impact participants who stated at the outset<span class="env-green">, “</span>I never remember my dreams<span class="env-green">”?</span> We hypothesized their dream recall would increase. |                                                              |
...
| -------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **Serial comma**                                   | Include a comma before the conjunction.   <br />My favorite composers are Beethoven, Brahms, Mahler<span class="env-green">, and</span> Mozart. | Do not include a comma before the conjunction.   <br />My favorite composers are Beethoven, Brahms, Mahler <span class="env-green">and</span> Mozart. |
...
- Include page numbers and continuous <span class="env-green">line numbers</span> in the manuscript file.
...
- ‼️ Be cautious when using Nord email as it might categorize the decision letter as "junk mail". <span class="env-green">Always check the junk mail folder after submission.</span>
