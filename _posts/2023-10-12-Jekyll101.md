...
1. First need to find the right `css` file path to overrule.
   ...
   ```bash
   rep "stylesheet.*css" _site/index.html
   ```
...
- If you want the script available on every page, put the call in your `default.html` or `post.html`layout file, whichever your post template is. Every page that uses that default.html layout will then call the script. That layout file can be found at `_layouts/default.html`.
...
- Note: Use Chrome $\rightarrow$ View $\rightarrow$ Developer $\rightarrow$ Inspect Element 
  - use `Console` to check if there is bug in your `js` code. If there is bug, `js` cannot run.
  - use `Application`  to check if the website load the `js` file at all.
...
- Equations load very slowly, maybe use `svg` to speed up loading?
