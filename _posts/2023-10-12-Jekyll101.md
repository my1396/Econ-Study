...
1. build with GitHub Pages
   ...
   GitHub now provides you with the option to use their in-house CI/CD product named *<span style='color:#32CD32'>GitHub Actions</span>* to *build and deploy (host)* your Jekyll site with complete control over the build environment and gemset.
...
1. I compared my `jekyll.yml` with [this script](https://github.com/0nyr/onyr-website/blob/main/.github/workflows/jekyll.yml), making sure my actions versions are consistent. 
...
- Sort by `update` if the object exists, otherwise sort by `date`. Might need to use Ruby or JavaScript. \
  ...
  But then you have to specify `update` tag in every post. `reverse` makes it descenging, i.e., from newest to oldest.
