...
1. the `tex2jax` processor
    ...
    It looks for mathematics within your web page (indicated by math delimiters like `$$...$$`) and marks the mathematics for later processing by MathJax.
...
The latest document for V3 may not be stable, some parts are missing. Refer to the [Documentation for V2](https://docs.mathjax.org/en/stable/) for a complete version.

<span class="env-orange">It is important to know which **MathJax version** you are using</span>, because the configuration, loading, and startup processes for MathJax version 3 are different from those of version 2 in a number of ways.

Examples of differences between V2 and V3:
...
- `tex` to configure the **TeX input processor** component. Check [HERE](https://docs.mathjax.org/en/latest/options/input/tex.html) for a full list of options. Commonly used options are:
  ...
  - `inlineMath` allows you to use single dollar signs (`$`) as <span class="env-green">in-line math</span> delimiters, in addition to the usual `\\(...\\)`.
    ...
    Note that when you use brackets and prarentheses, you need <span class="env-green"><u>double backslashes</u></span> (`\\(...\\)` or `\\[...\\]`) to escape backslashes.

    

  - `displayMath` an array of pairs of strings that are to be used as delimiters for <span class="env-green">displayed/block equations</span>.
  ...
  - <span class="env-green">`macros`</span> allows you to <span class="env-green">**define your own TeX commands**</span>. 
    ...
    - Note that the replacement string is given as a **JavaScript string literal**, and the backslash (`\`) has special meaning in javascript strings. 
      
      So to get an actual backslash in the string you must double it (`\\` this escapes the backslash), as in the examples above.
...
- `svg` output component specifies a global font cache for all expressions on the page, rather than a separate cache for each expression on the page. 
  ...
  - Other benefits of using `svg`: It won’t get pixelated at different screen sizes, like a raster image would. 
...
to <span class="env-green">**first** load your configuration file</span>, and <span class="env-green">**then** load the `tex-svg` component</span> from the `jsdelivr` CDN.
...
1. Define macros in the configuration file as shown above.
   Refer to [configmacros](https://docs.mathjax.org/en/v3.2/input/tex/extensions/configmacros.html#configmacros-options) for detailed syntax how to define macros.
2. Define macros directly in math environment using `\def`, `\newcommand`, `\renewcommand`, `\newenvironment`, `\renewenvironment`, and `\let`. 
...
While MathJax includes nearly all of the Plain TeX math macros, and many of the LaTeX macros and environments, not everything is implemented in the core TeX input processor. You may need to load additional extensions to get the functionality you want.

To enable one of the TeX extensions you need to do two things: 

1. load the extension with `loader/load`, and 
2. configure TeX to include it in its package setup. Add the extension name to the `tex/packages` array. 
   
   You can either add it to the default list using `'[+]': ['extensionname1', 'extensionname2']`, or remove it from the default list using `'[-]': ['extensionname1', 'extensionname2']`.

```js
// load TeX/LaTeX extensions
window.MathJax = {
  loader: {
    load: ['[tex]/color', '[tex]/upgreek']
  },
  tex: {
    packages: {'[+]': ['color', 'upgreek']}
  }
};
```
...
- [Load TeX and LaTeX extensions](https://docs.mathjax.org/en/v3.2/input/tex/extensions.html#tex-and-latex-extensions)
- [The TeX/LaTeX Extension List](https://docs.mathjax.org/en/v3.2/input/tex/extensions/index.html)
...
1. Since MathJax renders for the web and TeX is a print layout engine, there are natural limitations of which parts of TeX can be supported in a reasonable way. 
    ...
    It also defines the TeX delimiters `$$...$$` for displayed equations, but it does <span class="env-orange">**NOT**</span> define `$...$` as in-line math delimiters. 
    
    If you want to use `$...$` as in-line math delimiter, you need to configure it in the `tex` component.
...
Not all **fonts styles** (e.g., bold face) work as they should with MathJax math. E.g., `\mathbf` does not work for greeks.
...
[MathJax version 4](https://docs.mathjax.org/en/latest/output/fonts.html) includes support for a number of new font sets for MathJax, and changes the default font to one based on the New Computer Modern fonts, which offer support for a much larger range of characters than MathJax’s original TeX font set, but is consistent with the look-and-feel of the original MathJax TeX fonts. 

- I personally think Computer Modern family is too thin for web display, especially on low resolution screens.

Using web inspector tool, I found my website using
...
`MJXZERO` and `MJXTEX` were the original.

- `MJXZERO` for spacing and zero-width characters
- `MJXTEX` is the main default font for MathJax; top level container for all math elements. 

Note that the element names are not case-sensitive, so `mjx-tex` and `MJX-TEX` are the same.

To override the MathJax fonts, change `.mjx-tex` use css
...
<span class="env-green">`mjx-mtext`</span> controls the font for text in math mode, e.g., `$\text{Var}(X)$`.
To change the font family, use the following css

```css
mjx-mtext, 
mjx-mtext > mjx-utext {
  font-family: var(--bs-body-font-family), "Cambria Math" !important;
}
```

`var(--bs-body-font-family)` is the font family defined in the CSS variable `--bs-body-font-family` (Bootstrap body font family) in SCSS files.


I want to add `Noto Sans Math` to MathJax font family. This font supports a large number of Unicode math characters, and is freely available from Google Fonts.


Rule of thumb for choosing fonts for MathJax: Use <span class="env-green">serif fonts</span> in math environments as <span class="env-orange">sans-serif fonts have significant limitations</span> in their support for math symbols:

- Spacing is strange, look uneven or "off"
- Distinctions between symbols are not clear
...
In order to use `\color` command in MathJax V2, you need to load the `color` extension explicitly in your configuration file in the *extensions* array in the TeX block.
...
`color.js` loads the color extension; 

- Support color names, RGB, and grey-scale color spaces.

  ```latex
  $$
  \begin{split}
    y &= {\color{Aquamarine}{x^2}} + 1 \\
    y &= {\color{aquamarine}{\beta_1 x}} + \beta_0 \\
  \end{split}
  $$
  ```
  Will be rendered as
  ...
  Note that 
  - For MathJax V2, you need to put the command and math inside braces, e.g., `{\color{colorname} ... }`.
    - For MathJax V3, you can put the command either inside or outside the braces, e.g., `{\color{colorname} ... }` or `\color{colorname}{...}`.
  - Color names are **case-sensitive**, e.g., `\color{Aquamarine}` and `\color{aquamarine}` are the different colors.
    - You can run into "undefined color" error if you use the wrong case.
    - `\color{Aquamarine}` comes from `xcolor` package **`dvipsnames` color names**.
    - `\color{aquamarine}` comes from **CSS named colors**. You can find all CSS named colors [HERE](https://www.w3.org/TR/css-color-4/?utm_source=chatgpt.com#named-colors).

- This extension is <span class="env-orange">**NOT**</span> included in any of the combined configurations, and will not be loaded automatically, so you must include it explicitly in your configuration if you wish to use these commands, and have `\color` be compatible with LaTeX usage.

- For user defined color shortcuts, add `red: ["{\\color[RGB]{185,64,71}{#1}}", 1]` to your configuration file. 
  ...
  MathJax.Hub.Config({
    TeX: {
      extensions: ["color.js"],
      Macros: {
        red: ["{\\color[RGB]{185, 64, 71}{#1}}", 1], // 255-based integer RGB
      }
    }
  });
  ```

  Now you can use `\red{math}` to color `math` in red.

  ```latex
  $$y = \red{x^2} + 1$$
  ```

  - Note that `RGB` is **case-sensitive**. It uses 0 to 255 integer values.

    `rgb` uses decimal values between 0 and 1. Alternatively, you can use 
  ...
  MathJax.Hub.Config({
    TeX: {
      extensions: ["color.js"],
      Macros: {
        red: ["{\\color[rgb]{0.73, 0.25, 0.28}{#1}}", 1], // decimal rgb
      }
    }
  });
  ```

  - **Tip** for user defined color macros: use `rgb`/`RGB` instead of hexcode (`#RRGGBB`). 
    
    The hexcode starts with `#`, which can conflict with arguments in macros. 

  Refer to 
  
  - [MathJax V2 Defining TeX Macros](https://docs.mathjax.org/en/v2.6/tex.html#defining-tex-macros) for defining math macros.
  - [MathJax V2 TeX and LaTeX extensions](https://docs.mathjax.org/en/v2.6/tex.html#defining-tex-macros) for a list of all TeX extensions.

<figure style="display: flex; flex-direction: column; align-items: center; text-align: center;">
<img src="https://drive.google.com/thumbnail?id=1oXIGX2NIMGJ_1j2PW4m1grry63BCRRWr&sz=w1000" alt="color extension" style="display: block; margin-right: auto; margin-left: auto; zoom:80%;" />
<figcaption>The 68 standard colors known to dvips. The colors predefined when the package option <code>dvipsnames</code> is used. Note that color names are <strong>case-sensitive</strong>.</figcaption>
</figure>

See [HERE](https://drive.google.com/file/d/1UouvcyjBoj6Ig0fJx3IQGg2sD5pmgkbh/view?usp=drive_link) for a list of `svgnames` colors from the `xcolor` package.

See [Section 4 Color by Name in the `xcolor` package](https://ctan.uib.no/macros/latex/contrib/xcolor/xcolor.pdf) all available color names.
...
The `color` extension defines the `\color` macro as in the LaTeX *color* package, along with `\colorbox`, `\fcolorbox`, and `\definecolor`. 

- It declares the standard set of colors (*Apricot*, *Aquamarine*, *Bittersweet*, and so on), and provides the RGB, rgb, and grey-scale color spaces in addition to named colors.
- It colors everything that follows it, until the end of the current group or environment, or until another color is specified.
...
The `colorv2` extension re-defines the `\color` macro to take to arguments, one the name of the color (or an HTML color of the form `#RGB` or `#RRGGBB`), and the second the math to be colored.

- Note that `colorv2` is not loaded automatically when the *autoload* extension is used. To load the *color*extension explicitly, add `'[tex]/color'` to the `load` array of the `loader` block of your MathJax configuration, and add `'color'` to the `packages` array of the `tex` block.
...
### Adaptive color to dark/light mode

For a block equation, you can apply an html class directly to the `$$...$$` delimiters.

Ref: 

- [Eqn in colorbox](https://github.com/KaTeX/KaTeX/discussions/2553)
...
MathJax uses `tags: 'none'` by default, which means that auto-numbering was not performed.
...
- Note that for unnumbered equations, the `\begin{equation*}...\end{equation*}` needs to be <span class="env-green">enclosed in `$$`</span> so that it is rendered correctly.
...
1. Use <span class="env-green">`\tag*{\(\blacksquare\)}`</span>.
    ```latex
    $$
