...
- `tex` 
  ...
  - `tags: 'ams'`  tells the TeX input processor to use the AMS numbering rules.
    - Only certain environments (`\begin{equation}`) produce numbered equations, as they would be in LaTeX.
  
- `svg` to use a global font cache for all expressions on the page, rather than a separate cache for each expression on the page. 
