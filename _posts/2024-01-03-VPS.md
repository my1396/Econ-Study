...
4. Set up Shadowsocks-libev Server 
   ...
   **安装** Shadowsocks 代理。这一步通常叫作“部署服务器端”或者“搭建服务端”。这一步是在 VPS 上面的操作。
...
Shadowsocks-Python 版： `/etc/shadowsocks.json`
...
| local_address | the address your local listens 标准，不用改                  |
| local_port    | local port 标准，不用改                                      |
...
**Summary of all password you'll need to remember.**
...
<img src="{{site.baseurl}}/images/shell_launch.jpg" alt="Bandawagon" style="zoom:44%;" /> <img src="{{site.baseurl}}/images/shell_pass.jpg" alt="Bandawagon" style="zoom:44%;" />
...
完成电脑端的配置，此处我们介绍如何配置手机和平板。原则是用一个 Proxy Client，可以是 Shadowrocket 或者 V2BOX – V2ray Client。
...
便宜 ($6 /mon) 又容易设置，不需要自己配置 Server 和 Client。支持多设备在线。

流量限制：500 GB/mon 够用了

与自己搭建服务器，租服务器的价格比，每月$20, 3000G流量，确实自己搭建服务器单价划算。但是考虑到自己的学习成本，配置服务器时间成本，最容易的选择就是 Just My Socks.
...
`shadowsocks-libev` 不支持通过简单的修改配置文件来实现。

Currently Python and Go servers support multiple users. 可以用 Python and Go.
