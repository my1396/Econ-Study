...
4. **Set up Shadowsocks-libev Server** 
   ...
   Shadowsocks 简称 SS 或者 ss，有4个常见版本，`-R`, `-Python`, `-Go`, `-libre`。其中 `Shadowsocks-libev` 是目前唯一在持续更新且最为轻量的版本。`-Python` 版本是 OG 祖宗版本 (original release)，<u>支持多用户</u>。
   ...
   Make the following changes to the default configuration file:
      ...
      - Then <span style='color:#32CD32'>change `server_port` to other port numbers like 8888</span>. This is the port number that Shadowsocks will listen on. 此处的 `server_port` 跟租的 VPS SSH port 不是一个port, 事实上，VPS SSH port 在 SS 配置文档中<span style='color:#32CD32'>从来<u>没有用到过</u></span>。
...
5. **Install and Configure Shadowsocks-libev Client**
   ...
   - Specify the following:
     ...
     - <span style='color:#32CD32'>**server port (8888)**</span>, the port number that Shadowsocks is listening on. SS 服务器端口

     - <span style='color:#32CD32'>**password**</span>. SS 服务器密码
   ...
   The <span style='color:#32CD32'>**client config file**</span> (if there is one) will look like this:
   ...
   与 SS 服务端相比，客户端配置文档的主要不同为 `server`. 此处 IP 为 <span style='color:#32CD32'>VPS Server SSH IP</span>。其他参数，两个配置文件 (原则上) 要保持一致，如`mode`, `local_port`, `password`, `method`.
...
| password      | password used for encryption, clients will use to connect to the Shadowsocks server. 用户自定义，注意此处 `password` 非 VPS Server Root Password. ❌ <br />Root password 在配置 SS 中跟本用不到。 |
...
1. **查看 VPS 基本信息**
   ...
   2. 之后选择左侧的”Root password modification“，进入搬瓦工的修改 ROOT 密码栏目，再点击”Generate and set new root password“就可以修改并设置搬瓦工的 ROOT 密码了。
...
## <span class="env-green">Just My Socks</span>
...
在购买 Just My Socks 时，我们可以使用 Just My Socks <span class="env-green">优惠码</span>: **JMS9272283** 获取 5.2% 循环优惠，输入优惠码后点击 Validate Code 即可使用优惠码，点击 Checkout 付款。

Just My Socks 官网 (用于购买): <https://justmysocks.net/members/clientarea.php?action=services>

中文教程:

- [GitHub 教程](https://github.com/killgcd/justmysocks/tree/master)
- [教程汇总，优惠码](https://bwgjustmysocks.com)
- [新手入门](https://bwgjustmysocks.com/32.html)
...
Just My Socks uses [`Jamjams`](https://jamjamsapp.com) <img src="https://drive.google.com/thumbnail?id=18zgAVH1wNxB5dK9ulQbwMt-17h7l0Vgr&sz=w100" alt="" width="40" height="40" /> to set up iPhone and Mac. Just add the <span class="env-green">subscription link</span> to Jamjams, and it will automatically configure everything for you.


<img src="https://drive.google.com/thumbnail?id=1to25gfi_BOcVNOVu1IhLy1pe-DQ7WPC-&sz=w1000" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:80%;" />
...
详细使用教程<a href="{{site.baseurl}}/2024/01/03/VPS2.html#jamjams-iphone">此处</a>

Below is the Jamjams menu bar, you can 

<img src="https://drive.google.com/thumbnail?id=1QrXRBWle1fETeoxgCDgiGGRgiYnsfZvw&sz=w1000" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:50%;" />

- **Show Jamjams main panel**
  
  Here you can add your subscription link, and manage your servers.

  - If you don't have any subscription yet, you will be able to add one here.

  - If you have already added a subscription before, click the existing subscription, a list of server will show up. Click the <kbd>+</kbd> sign to add your new subscription(s).

  <img src="https://drive.google.com/thumbnail?id=1_RuAHaUV-aEAqPP56-VVsDyTqjlxk5tF&sz=w1000" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:40%;" />


- **Connect / disconnect the proxy**

  This turn on/off the proxy.

  Note that quiting Jamjams will not turn off the proxy. You need to manually disconnect the proxy before quiting Jamjams.

- Open Jamjams <a href="{{site.baseurl}}/2024/01/03/VPS2.html#preferences">**Preferences**</a> (⌘,)
  
  You can go to the `Subscriptions` tab to add your subscription link here too.
  
  <img src="https://drive.google.com/thumbnail?id=1y62pyKnnru9UuISY2pD-ELw6r07llSzF&sz=w1000" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:50%;" />

- Check your server info
  
  Here will show a list of your servers.
  You can quickly switch between servers by clicking on the server name.
...
A: An IP address is a string of numbers separated by periods. IP addresses are expressed as a set of four numbers — an example address might be 192.158.1.38. Each number in the set can <u>range from 0 to 255</u>. So, the full IP addressing range goes from 0.0.0.0 to 255.255.255.255.
...
Q: Why can I not use Google Scholar?

```
403. ​That’s an error.
Your client does not have permission to get URL / from this server. (Client IP address: 185.xxx.yy.zzz)
Please see Google's Terms of Service posted at https://policies.google.com/terms
```

A: Your server IP is blacklisted by Google Scholar. If Google finds your IP abusive, you have to switch to another IP to try or change your host provider. Use ipv6 instead of ipv4 to visit google Scholar by changing the hosts file may work.

___

Q: IP 被封怎么办?

A: 表现是IP Ping不通。解决方式: 换机房 (Migrate to another datacenter)。

___

Q: 如何查看自己的IP是否被封？

A: 前往下面网址: [port.ping.pe](http://port.ping.pe/). 然后输入你的 IP:端口 (如 `97.64.81.187:22`)，开始测试，如下图所示。

<img src="{{site.baseurl}}/images/ip.png" alt="Bandawagon" style="display: block; margin-right: auto; margin-left: auto; zoom:80%;" />

**结果一: **如果出现下面这样的，境外的显示绿色的 successful，大陆的显示红色的 failed，那么你的 IP 或者端口就是被封了。先换个端口试一下，如果换了端口还是这样就是 IP 被封了。

**结果二: **大陆、境外都是 successful，那么你的端口没有任何问题。

**结果三: **大陆、境外都是 failed，那么你的端口没有开放，检查一下你的 VPS 的设置吧。

___

### Troubleshoot

Q: Google Chrome is on and off constantly.

A: Check proxy settings. Turn off Proxy SwitchyOmega if you are using it. If this does not help, re-install Chrome. Re-installing Chrome won't affect your extensions, settings, and bookmarks data.
...
This feature allows you to "teleport" your VM to a different datacenter with zero data loss and minimal downtime – just a few seconds to a few minutes.

Q: Will my IP address change?

A: **IPv4** address will be changed. If you host websites or services that rely on DNS, be sure to update the DNS records accordingly. Additionally, if any of your services bind to a static IPv4 (rather than 0.0.0.0 or localhost 127.0.0.1), you'll need to update their configuration files to reflect the new IP.

**IPv6** subnet, if enabled, will stay the same.*We will transparently forward all traffic to/from the new location, so you can continue using the existing IPv6 address. This may increase IPv6 latency.



The entire migration process may take anywhere from a minute to a few hours, depending on the number of files in this VM and their size.
However, this VM will be operational while migration is performed. It will be shut down for a few seconds to few minutes during the last stage of the process.



Issue: Migration backend is currently not available for this VPS. Please try again in 10-15 minutes.

25/02/01 等了一整天，仍然不可以换机房。

___

#### Request IP change

References: <https://bandwagonhost.com/ipchange.php>

<https://www.bandwagonhost.net/1925.html>

The cost of IP change submitted via this form is $7.39. After payment is received we will perform the following:

- We will assign a new (random) IPv4 address from the available pool
- We will verify that it is accessible from all continents
- After that we will delete your old IP address.
...
We have 30-day money back guarantee, subject to our Terms of Services. Please submit a request for full refund by navigating to:

https://bandwagonhost.com/refund.php

The system will enter a request to our billing department on your behalf.

Some service does not provide automatic refund. $\rightarrow$ Need to submit a [support request](https://bandwagonhost.com/tickets) for manual processing.

Submitted request 2025-02-02.
...
配置了多端口，相当于一个主IP (Main IP) 下，有不同的子 IP。比如你的主 IP 为 `43.45.43.21`, 经过如上配置，你就会获得 `43.45.43.21:9000`, `43.45.43.21:9001` ... 等五组子 IP。配置后访问方式为`主IP:端口`，同一个主 IP，不同的端口，对应不同的子IP。
...
2. 复制和修改配置文件
   ...
   Shadowsocks-libev 版的配置文件是 `/etc/shadowsocks-libev/config1.json` ，默认里面的内容如下: 
...
3. 修改 iptables 规则 (Not sure if this is necessary. There are tutorials that do not include this step)
...
4. Use the following codes to create instances for each user: 

   启动 Shadowsocks-libev 版服务端

   需要注意的是，多个配置文件一定要注意的是，配置的端口和 `pid` 文件一定是不同的！而密码及加密方式，甚至服务器IP都没有限制。

   假设是以 root 用户登录的，那么需要分别以 user1 和 user2 运行 Shadowsocks-libev 版服务端。多实例运行.
   命令如下: 

   ```bash
   ss-server -c /etc/shadowsocks-libev/config1.json -f /tmp/pid1
   ss-server -c /etc/shadowsocks-libev/config2.json -f /tmp/pid2
   ```
   ...
   xxx      1344     1  0 20:12 ?        00:00:00 ss-server -c /etc/shadowsocks-libev/config1.json -f /tmp/1.pid
   xxx      1491     1  0 20:24 ?        00:00:00 ss-server -c /etc/shadowsocks-libev/config2.json -f /tmp/2.pid
   ```
