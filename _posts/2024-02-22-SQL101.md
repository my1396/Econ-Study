...
<span style='color:#32CD32'>**Azure SQL Documentation**</span>: <https://learn.microsoft.com/en-us/azure/azure-sql/?view=azuresql>
...
- `WHERE` to filter which rows to subset; need to be <span style='color:#32CD32'>located after all tables</span> in use.
...
A: They come in handy if the names of your columns or tables coincide with <span style='color:#32CD32'><u>reserved SQL keywords</u> or <u>contain spaces or hyphens</u></span>. 
...
SELECT price.*, sym.ticker_region, cov.proper_name, pct_return.one_day_pct
    FROM fp_v2.fp_basic_prices AS price
    JOIN fp_v2.fp_total_returns_daily As pct_return
		    ON price.fsym_id = pct_return.fsym_id AND price.p_date = pct_return.p_date
...
SELECT top 100 * 
	...
	WHERE p.p_date BETWEEN '2020-08-16' AND '2020-08-30' -- change date range
```
