...
2. Use `sym_v1.sym_coverage` to find regional level `fsym_id`
   ...
   ```sql
   -- Given security level fsym_id, look up regional level
   SELECT * FROM sym_v1.sym_coverage
       LEFT JOIN sym_v1.sym_ticker_region
           ON sym_v1.sym_coverage.fsym_id = sym_v1.sym_ticker_region.fsym_id
       WHERE sym_v1.sym_coverage.fsym_security_id IN ('MC4TDP-S', 'NV2X6X-S', 'PX9JZ6-S', 'S8TF9W-S', 'SY7WGC-S') 
           AND sym_v1.sym_coverage.fref_listing_exchange = 'NYS'
   ```

   The final condition add a filter for exchange to be `NYS` exchange.

   The column you need is `fsym_regional_id`. `proper_name` is useful too.

   Filter by unique `fsym_security_id`.

   Tip: <span style='color:#FF9900'>`LEFT JOIN` is safer than `(INNER) JOIN`.</span> Sometimes, you have 0 row returned. You can change to `LEFT JOIN` and see what is the problem. 

   Ex.2

   ```sql
   -- Given security level fsym_id find regional level id
   SELECT 
       cov.*,
       tr.ticker_region 
...
3. Get price (`fp_v2.fp_basic_prices`) and return (`fp_v2.fp_total_returns_daily`) data given regional level `fym_id`.
   ...
   2. recommend to <span style='color:#00CC66'>**use `.sql`**</span>. It is much <span style='color:#00CC66'>faster to run the query and save to csv</span>!
...
| cusip   | Current, or most recent CUSIP assigned to the security.<br />Mostly used in the US and Canada. |
...
- The **Insight** Score measures a company’s <u>longer-term</u> ESG track record.
  ...
  Scores are <u>less sensitive to daily events</u> and reflect the enduring performance record of a company over time. Scores are derived using an exponentially-weighted moving average of the Pulse, defined below, and the half-life of an event’s influence on the overall score is 6 months.
