...
**`sym_v1.sym_isin`** contains <span style='color:#32CD32'>ISIN</span> to FactSet Permanent Identifier (FSYM_ID) mappings for equity and fixed income securities included in the FactSet Symbology universe.

ISINs are mapped to <span style='color:#32CD32'>security level</span> FactSet Permanent Identifiers. 
...
| -------------------------------------------- | ------------------------------------------------------------ |
| <span style='color:#32CD32'>`fsym_id`</span> | FactSet <span style='color:#32CD32'>security level</span> identifier assgined to a security |
...
3. Get price (`fp_v2.fp_basic_prices`) and return (`fp_v2.fp_total_returns_daily`) data given regional level `fym_id`.
   ...
   2. recommend to <span style='color:#32CD32'>**use `.sql`**</span>. It is much <span style='color:#32CD32'>faster to run the query and save to csv</span>!
...
| <span style='color:#32CD32'>`universe_type`</span> | Code denoting whether the security is considered equity (EQ) or fixed income (FI) |
