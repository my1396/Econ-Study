...
```html
<span class="smallcaps"><strong>Small caps</strong></span>
```
...
See [Pandoc Documentation: Extensions](https://pandoc.org/MANUAL.html#extensions) for supported extensions in Pandoc markdown. You can click Extensions on the TOC on the right to have an overview.

**Some commonly useful extensions:**

| Extension                   | What it does                                |
| --------------------------- | ------------------------------------------- |
| `markdown_in_html_blocks`   | Process Markdown inside HTML block elements |
| `tex_math_dollars`          | `$...$` for LaTeX math                      |
| `smart`                     | Curly quotes, em-dashes, ellipses           |
| `footnotes`                 | `[^1]` style footnotes                      |
| `pipe_tables`               | GitHub-style pipe tables                    |
| `strikeout`                 | `~~strikethrough~~`                         |
| `superscript` / `subscript` | `x^2^` / `x~2~`                             |
| `raw_html`                  | Pass raw HTML through to output             |
| `bracketed_spans`           | `[text]{.class}` syntax                     |
| `fenced_divs`               | `:::` div blocks                            |


Use `+extension` to enable and `-extension` to disable.
