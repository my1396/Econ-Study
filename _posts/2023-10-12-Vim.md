...
A: Enter visual mode (⇧V) → select your lines → enter colon (`:`), `'<,'>` will auto add, which means for the selected lines → Type <span style='color:#008B45'>`norm I<space><space>`</span> (norm, capital I, two spaces)→ hit Enter.
