...
| ----------- | ---------------------------------------------------------- |
| `{}`        | parts of the command which must appear in the command.     |
| `{motion}`  | A command that moves the cursor. E.g., `w` (move to the beginning of the next word), `e` (move to the end of the current word), `j` (move down one line), etc. |
...
| `h/j/k/l`                   | ←/↓/↑/→ <br />To move several lines, just add the number before the letter. E.g., move 5 lines down: `5j`. |
...
| `d{motion}`                 | Delete text that is moved over. E.g., `dw` deletes a word, `d$` deletes to the end of the line, `d^` deletes to the beginning of the line. | 
...
| `db`                        | Delete the <span class="env-green">previous</span> word. |
...
- `d` must be followed by a motion command, it cannot conduct deletion by itself.
- `x` itself is a command, it deletes the character under the cursor.
...
Q: How to <span class="env-green">Undo and Redo</span>?  
A: In command mode, 
...
Method 1 – Normal Command Mode with `:norm`
...
Method 2 – **Visual Block Mode** (⇧ + <code>V</code>) ✅
...
-   Press ⇧ + <code>V</code> to enable Visual Block Mode.
    -   Using the up and down arrow key, highlight the lines you wish to comment out.
    -   You can also use `j` and `k` to move down and up, respectively. 
        
        `5j` will move down 5 lines, `3k` will move up 3 lines, etc.

-   Once you have the lines selected, press the `SHIFT` + `I` keys to enter insert mode.
    -   <span class="env-green">⇧ + <code>I</code></span> will allow you to insert text at the *beginning* of the highlighted lines.
    -   <span class="env-green">⇧ + <code>a</code></span> will allow you to insert text at the *end* of the highlighted lines.
...
2. Press ⇧ + <code>V</code> to enter Visual Block mode
...
- This approach can be used to add same characters in the middle of multiple lines as well. 
 
  Method 1 inserts at the beginning of the line, Method 2 can insert anywhere.

ref: [Inserting text in multiple lines](https://vim.fandom.com/wiki/Inserting_text_in_multiple_lines)
...
| `.`        | any character except a newline  | `.*` means zero or more of any character; <br>`.+` means one or more of any character.   |
| `[A-Za-z]` | all letters *(uppercase and lowercase)*    |                                             |
| `\d`       | any digit character, equivalent to `[0-9]` | `\d\d` matches any two digits, `25`, `17`, etc. |
| `\d+`      | one or more digits                       | `\d+` matches `1`, `123`, `000`, etc.       |
| `\D`       | One character that is not a digit        | `\D\D` matches any two characters that are not digits, such as `ab`, `!@`, etc. |
| `\*` or `\.` | `\` means to escape; | `\*` matches the literal character `*`, `\.` matches `.` |
| `?`        | zero or one occurrence of the preceding character | `colou?r` matches both `color` and `colour`. |
...
| `(...)`    | captureing group | `(abc)+` matches `abc`, `abcabc`, `abcabcabc`, etc. The parentheses create a capturing group that can be referenced later in the regex or in the replacement string. |
| `(?:...)`  | non-capturing group | `(?:abc)+` matches the same strings as `(abc)+`, but does not create a capturing group. This is useful when you want to group parts of the regex without needing to reference them later. |
