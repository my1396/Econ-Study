...
```html
{% raw %}<img src="{{site.baseurl}}/images/document-link-svgrepo-com.png" alt="" style="display: inline; height: 1.5em; vertical-align: middle;" />{% endraw %}
```

Comment: Not readable under light mode. Fix this later. [26-01-28]
...
Save as svg file: Right click on the symbol, select "Copy Image As", then choose "SVG", set "Point Size" to 1000 to get a high resolution svg file. Click "Copy Image", go to Finder, ⌘+V to paste the svg to the current folder. Then you can use the svg file as needed.

<img src="{{site.baseurl}}/images/SF symbol save image.png" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:50%;" />

You can use SF Symbols toghether with [ICON Composer](https://developer.apple.com/icon-composer/), which allows you to create Liquid Glass icons.
