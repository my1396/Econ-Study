...
| `<u>`            | <span style='color:#32CD32'><u>underline</u></span> text     |
...
`style` attribute specifies an <span style='color:#32CD32'>**inline style**</span> for an element, such as color, font, size. It allows you to specify multiple attributes at one time. It is equivalent to use `font-size="12px"`. 冒号赋值，分号分隔 properties。
...
- Now we run into the usage of the word **property**. Each property consists of a property-key and it’s associated VALUE, separated by a colon `:`. The syntax is `property-key:value`. If you have more then one property you use the <span style='color:#32CD32'>semi-colon `;` as separator. </span> E.g., 
  ...
  - `style = “font-size:12px; background-color:blue;”` to sepecify multiple properties.
    ...
    - multiple properties are separated by <span style='color:#32CD32'>spaces</span>.
...
- There are two types of font family names:
  ...
  A sans-serif font is <span style='color:#32CD32'>more readable</span> especially on lower-resolution screens.
...
An ID selector selects an element based on its ID attribute. For example, `#toc` will select the element that has the ID `toc`. Please note that this selector only works if the value given in the selector matches the element’s ID attribute <span style='color:#32CD32'>exactly</span>. ID is case-sensitive.
...
`[title~="flower"]`(`[attribute~="value"]`): Selects all elements with a title attribute that contains a <span style='color:#32CD32'>space-separated</span> list of words, one of which is "flower".
...
- <span style='color:#32CD32'>`max-width: 100%; white-space: nowrap;`</span> prevent the text from wrapping.
...
- `table-layout: auto|fixed|initial|inherit;` property defines the algorithm used to lay out table cells, rows, and columns.
  ...
  - `fixed` sets a fixed table layout algorithm. This makes table render fast. 
    ...
    The table and column widths are set by the widths of table and col or by the width of the <span style='color:#32CD32'>first row</span> of cells. Cells in other rows do not affect column widths. 
    
    If no widths are present on the first row, the column widths are <span style='color:#32CD32'>divided equally</span> across the table, <span style='color:#32CD32'>regardless of content</span> inside the cells.
...
- `<td><div style='width: 150px;'>Text to break here</div></td>`  let you fix the <span style='color:#32CD32'>width of the column</span> containing the cell at 150px.
