...
| Tag              | Description                                                  |
| :--------------- | :----------------------------------------------------------- |
| `<!--...-->`     | Defines a <span style="color: #00CC66;">comment</span>; support line breaks; Quarto also use HTML comments; |
...
| `<h1>` to `<h6>` | headings; <br/> Bootstrap framework commonly adds `.h4` as a utility *class* to make non-heading elements look like h4 headings, e.g., `<p class="h4">`. |
| `<img>`          | image                                                        |
...
| `<strong>`       | Show importance. Most browsers use <span style="color: #00CC66;">**boldface**</span> for enclosed text, but might be different.  |
| `<b>`            | Alternative way to make text **bold**                        |
| `<span style="font-weight: bold;">` | Inline css to make text **bold**.         |
| `<em>`           | use <span style="color: #00CC66;">*italic*</span> font, show emphasis |
| `<u>`            | <span style="color: #00CC66;"><u>underline</u></span> text |
| `<tt>`           | <span style="color: #00CC66;"><tt>monostyle/typewriter</tt></span>, commonly used for code |
...
| `<pre>`          | Defines preformatted text; commonly used for monospaced source code; preserves whitespace like space, tabs, and newlines. |
| `<p>`            | Paragraph; collapse consecutive spaces and line breaks into one. |
...
- `<link>`: most often used to link to an **external style sheet**
  ...
  - A `<link>` element can occur either in the `<head>` or `<body>` element, depending on whether it has a [link type](https://html.spec.whatwg.org/multipage/links.html#body-ok) that is **body-ok**. `rel` attribute specifies the link type.
    ...
    - `rel="icon"` is used to specify a favicon for a website. The favicon will show in tabs, bookmarks, address bar.

      `rel="shortcut icon"` is used in older browsers.

      `rel="apple-touch-icon"` is used to specify an icon for IOS devices (iPhone/iPad). When you add your site to home screen, this will be used.
...
- side-by-side image in one row
  
  ```html
  <div style="display: flex; gap: 10px; justify-content: center;">
  <img src="https://drive.google.com/thumbnail?id=&sz=w1000" alt="" style="width: 48%; object-fit: contain;" />
  <img src="https://drive.google.com/thumbnail?id=&sz=w1000" alt="" style="width: 48%; object-fit: contain;" />
  </div>
  ```

  `<div style="display: flex">` creates a flex container, which allows you to arrange its child elements (the images) in a row or column. By default, it arranges them in a row.
  
  `object-fit: contain` property specifies how the contents should be resized to fit its container. Keeps its aspect ratio.
  Fits inside its box. Doesn’t get cropped or stretched.
...
- inline image
  ...
  ```
  Can use absolute size as well:

  ```html
  <img src="..." heigt="30" width="30" alt="巡航" />
  ```

  This sets the image height and width to be 30 pixels, ideal for inline <span style="color: #00CC66;">icon images</span>.

--------------------------------------------------------------------------------

**Embed images using <span style='color:#00CC66'>relative path</span>**

Images can be stored in a folder named `images` or `assets` within your project directory. Then you can use relative path to embed the image as follows.
...
An image hosted on GitHub has the following URL format:

```
https://github.com/overleaf-workshop/Overleaf-Workshop/blob/master/docs/assets/screenshot-online-collaborators.png
```

To embed the image in your HTML, you need to use the **raw** version of the image URL. 
You need to <span class="env-green">replace `blob` with `raw`</span>.

The raw URL format is:

```
https://github.com/overleaf-workshop/Overleaf-Workshop/raw/master/docs/assets/screenshot-online-collaborators.png
```

Then you can use the raw URL in the `src` attribute of the `<img>` tag.
...
It will generate a viewer widow with the specified width and height. You can scroll to view the whole pdf file.
...
- <span class="env-green">`font-weight: normal | bold;`</span> 加粗 specifies the weight of a font: boldface or normal
...
- <span class="env-green">`vertical-align: top | middle | bottom;`</span>  specify the vertical alignment of text.
  ...
  - `text-top`, <span class="env-green">`text-bottom`</span>: aligns the top or bottom of the element with the top or bottom of the parent element's font. → Sometimes `top`/`bottom` does not work as expected, so you can use `text-top`/`text-bottom` instead.
...
- The `font-family` property can hold several font names as a <span style="color: #00CC66;">"fallback"</span> system. If the browser does not support the first font, it tries the next font. Separate each value with a comma.
  ...
  - System font
    ...
    body {
      font-family: -apple-system, BlinkMacSystemFont, "Helvetica Neue", Helvetica, Arial, "Source Han Sans SC", "PingFang SC", "Microsoft YaHei", sans-serif;
    }
    ```

    - `-apple-system` targets `San Francisco` in Safari and Firefox. 
    - `BlinkMacSystemFont` is the equivalent for Chrome on Mac OS X.
    - `system-ui` represents the default UI font on a given platform.
   
    To support CJK characters, add fonts that are compatible with CJK characters, e.g., "Source Han Sans SC" (思源等宽), "PingFang SC" (苹方), "Microsoft YaHei" (微软雅黑).
    
    ```css
    /* System Fonts with system-ui */
    body {
      font-family: -apple-system, system-ui, BlinkMacSystemFont, "Helvetica Neue", Helvetica, Arial, "Source Han Sans SC", "PingFang SC", "Microsoft YaHei", sans-serif;
    }
    ```
...
**Issue:** 除了黑体之外，Safari 其余中文字显示全都显示为宋体。Fonts show properly in Chrome though.  

**Cause:** Check the font's location in Font Book app. Usually, these fonts are installed in a weird location other than `/Library/Fonts/`, and the location field is hidden, you cannot right-click to Show in Finder either. 
<span class="env-orange">These fonts are NOT properly loaded by Safari</span> (fingerprinting protection), which causes the issue. 
But Chrome can load the font files properly.

👉 **Fix:** For Safari compatibility, <span class="env-green">need to explicitly load the font file using `@font-face` in CSS</span>. 


Put the font files in `assets/fonts`, then add the following code to CSS. 

```css
{% raw %}@font-face {
  font-family: 'WebKaiti';
  src: url('{{ site.baseurl }}/assets/fonts/STKaiti.ttf') format('truetype');
  font-weight: normal;
  font-style: normal;
  font-display: swap;
}{% endraw %}
...
`@font-face` tells the browser to load a font from a specific file URL rather than relying on it being installed on the user's system.

Note that you need to specify the path to the font file using baseurl tag `{% raw %}{{ site.baseurl }}{% endraw %}`. You can use Web inspector tool network tab to check if the font file is loaded properly. If the font file is missing, you will see a 404 error in the network tab.

--------------------------------------------------------------------------------


**Chinese fonts preview**

<p style="font-family: 'WebKaiti', serif !important;">字体测试：华文楷体 (WebKaiti)</p>

<p style="font-family: 'STKaiti', serif !important;">字体测试：华文楷体 (STKaiti)</p>

Note that in `assets/js/font-replacement.js`, there is a map for mapping system font names to local font names defined by `@font-face`. 
So if you use `STKaiti` in your CSS, it will be replaced by `WebKaiti`, which is the font that is properly loaded by Safari.

```js
// Font mapping: 
// installed font in Font Book -> local font in assets/fonts/ folder
const fontMap = {
  'STKaiti': 'WebKaiti',
  'Xingkai': 'WebXingkai',
  'Kaiti SC': 'WebKaiti',
...
This mapping is necessary for Safari compatibility as Safari cannot load the font files properly.

<p style="font-family: 'Xingkai', serif !important;">字体测试：华文行楷 (Xingkai SC)</p>

<p style="font-family: 'Hannotate', serif !important;">字体测试：手札体-简 (Hannotate SC)</p>

<p style="font-family: 'Source Han Sans SC', 'PingFang SC', sans-serif;">字体测试：思源等宽 (Source Han Sans SC)</p>

<p style="font-family: STSong !important;">字体测试：华文宋体 (STSong)</p>

<p style="font-family: 'Heiti SC' !important;">字体测试：黑体 (Heiti SC)</p>

--------------------------------------------------------------------------------

**Unresolved: Automatic fonts for English-Chinese websites**
...
- `pt` point size. A point is about 0.3515 mm. Better support for <span class="env-green">print media</span>. Not recommended for screen display.

- `px` pixel size; 16 px = 12 pt = 1em = 100%; use `px` whenever possible for best <span class="env-green">screen display</span>;
...
- `rem` means "root em", relative to the root element (`<html>`), usually 1rem equals 16px (the browser default). 
...
Easiest and most robust. But does not look as good as vector icons (svg).
...
If you download svg files, use text editor to open the `.svg` file. You should set `stroke="none"` and `fill="currentColor"` in the `<svg>` tag to make sure the icon inherits the text color of the surrounding text.

```css
/* CSS to invert svg images in dark mode */
img[src*=".svg"] {
  filter: none;
}
...
In text, use the following code to include the svg icon:

```html
{% raw %}<img src="{{site.baseurl}}/images/document-link-svgrepo-com.svg" alt="" style="display: inline; height: 1.5em; vertical-align: middle;" />{% endraw %}
```

This inserts a document link icon: <img src="{{site.baseurl}}/images/document-link-svgrepo-com.svg" alt="" style="display: inline; height: 1.5em; vertical-align: middle;" />

Comment: Not readable under dark mode. Fix this later. [26-01-28]

### SF Symbols

[SF Symbols](https://developer.apple.com/sf-symbols/) is a library of symbols that integrate seamlessly with the San Francisco system font. 
You can copy and paste SF Symbols directly into other apps, save as png or svg files. 

Save as svg file: Right click on the symbol, select "Copy Image As", then choose "SVG", set "Point Size" to 1000 to get a high resolution svg file. Click "Copy Image", go to Finder, ⌘+V to paste the svg to the current folder. 

<img src="{{site.baseurl}}/images/SF symbol save image.png" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:50%;" />

In your document, insert the svg file as needed. E.g., if you have saved the svg file as `info.circle.svg` in the `images` folder, use the following code to include the icon:

```html
{% raw %}<img src="{{site.baseurl}}/images/info.circle.svg" alt="" style="display: inline; height: 1.5em; vertical-align: middle;" />{% endraw %}
```

It will show an info circle icon: <img src="{{site.baseurl}}/images/info.circle.svg" alt="" style="display: inline; height: 1.5em; vertical-align: middle;" />

Advanced usage: You can use SF Symbols toghether with [ICON Composer](https://developer.apple.com/icon-composer/), which allows you to create Liquid Glass icons.
...
Note that `//` is not a valid comment in CSS. It will be treated as an invalid declaration and ignored by the browser.

However, you can use `//` in SCSS preprocessors like Sass or Less, but not in plain CSS.
...
**Use <span class="env-green">variable</span> to assign values in CSS:**

`var(--variable-name, fallback-value)` is a function that retrieves the value of a CSS variable. If the variable is not defined, it uses the fallback value provided as the second argument.

You can define CSS variables within a selector, typically in the `:root` selector to make them global:
...
This example sets the background color of the body using a CSS variable `--main-bg-color`. If the variable is not defined, it falls back to `lightblue`.

Note that custom properties (CSS variables) are case-sensitive and must be prefixed with two hyphens (`--`).
...
```css
/* Define custom CSS variables in the :root selector for global scope */
:root {
  --main-bg-color: #222730;
  --main-text-color: #cae1f4;
}

/* Then you can use these variables throughout your CSS */
body {
...
SCSS variables are defined using the `$` symbol and are processed at compile time. They cannot be changed dynamically in the browser.
...
After compilation, the output CSS will be:
...
The variable `$main-bg-color` no longer exists anywhere.

By contrast, CSS variables are defined using the `--` syntax and can be accessed and modified at **runtime** in the browser. Runtime means that variables lives in the CSS delivered to the browser, and the browser resolves it while rendering the page. You can even change the value of a CSS variable using JavaScript on the fly.
...
Q: When I use Dark Reader extension, how can I check the variable values used in the page?  

```css
body {
  /* text color */
  color: var(--darkreader-text--fgColor-default, var(--darkreader-text--color-fg-default));
  /* background color */
  background-color: var(--darkreader-bg--bgColor-default, var(--darkreader-bg--color-canvas-default));
}
```

A: Use Web Inspector, go to the `Elements` tab, then in the <span class="env-green">`Styles`</span> pane, scroll down to the bottom to find a list of variable definitions. Alternatively, you can hover over the variable name in the `Styles` pane to see its value in a tooltip.

- "Copy declaration" will copy the property and its values. It won't copy the selector.
  
  E.g., `color: var(--darkreader-text--fgColor-default, var(--darkreader-text--color-fg-default));`

- "Copy rule" will copy the whole block of CSS rules including that element.
  
  It can get quite long if there are many properties defined for that element.
...
If you don’t see the color changing on your site, then it means you either have the wrong selector OR it means that something else is overriding your color choice (a strikethrough line means sth is overridden). You can force it to go with your choice by adding `!important` to the end of line you added, <span class="env-green">right before the `;`</span>. 

Note: <span class="env-orange">NO comma</span> between `!important` and the value.
...
- **If it is a heading**
  ...
  Note that if you use **the default markdown tag** for headings, you need to make sure the link text matches the heading text exactly, with spaces replaced by hyphens (`-`), and all letters in **lowercase**.

  It is a headache to remember the rules and this approach is brittle, because if you change the heading text, the link will break.

  ✅ A recommended way is to <span style="color: #00CC66;">add a custom id `{#my-custom-id}` to the heading.</span>
  ...
  (I personal think this is the most robust way of referencing headings)
  ...
  Note that <span class="env-green">**a blank line is required**</span> between the `<a>` tag and the heading for the heading to render correctly. 
  Otherwise, the heading will NOT be recognized as a heading.

  `<a name="my-custom-id">` is equivalent to `<a id="my-custom-id">` for defining a named anchor. 
  `id` and `name` are interchangeable in this case.
...
> 定义 HTML 标签时，`id` 和 `name` 是没有`#`的。  
> 引用标签时，必须要在 `id` 和 `name` 前加 `#` 。  
> 
> HTML 和 Markdown 方式都可以定义和引用标签，并且可以混用，及用 HTML 定义标签，用 Markdown 引用标签，反之亦然。  
> 个人推荐用 HTML 定义标签，Markdown 引用。感觉这样语法最简单。✅
...
<span class="env-green">Inline code that spans multiple lines</span>: use `white-space: pre` and then `&nbsp;` for indentation.
...
**Disable auto wrapping for code blocks**

Sometimes you may want to disable word wrap for certain code blocks, e.g, output from software. If you force soft wrap, the structure will be broken, making it hard to read. In this case, you can define a class <span class="env-green">`.nowrap`</span> as follows.
...
Wrap your code block with `<pre class="nowrap"><code>...</code></pre>` to disable soft wrap.
...
<table style="width:100%; border-collapse: collapse;">
  ...
  <tbody>
    <tr>
      ...
      <td>Default value. Sequences of whitespace will collapse into a single whitespace. Text will wrap when necessary.</td>
    </tr>
    <tr>
      <td><span class="env-green" style="font-weight:bold;"><code>pre</code></span></td>
      <td>Whitespace is preserved <u><em>exactly</em></u> by the browser. Text will only wrap on line breaks. <br/>
      Acts like the <code>&lt;pre&gt;</code> tag (for source code blocks) in HTML.</td>
    </tr>
    <tr>
      <td><code>pre-wrap</code></td>
      <td>Whitespace is preserved by the browser. Text will <span class="env-green">wrap</span> when necessary, and on line breaks.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        Sequences of whitespace will collapse into a single whitespace. Text will <span class="env-green">never wrap</span> to the next line. The text continues on the same line until a &lt;br/&gt; tag is encountered. <br/>
...
Most often used values are `pre`, and `no-wrap`.

- `pre` useful for code blocks that contains multi-lined regression output. This will preserve all whitespaces exactly, won't collapse your spaces for instance.
- `no-wrap` will collapse spaces into one. Useful for inline code.
...
<table style="width:100%; border-collapse: collapse; table-layout: fixed;">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Deprecated. To prevent overflow, words may be broken at arbitrary points.</td>
    </tr>

    <tr>
      <td><code>break-all</code></td>
      <td>To prevent overflow, words may be broken at any character.</td>
    </tr>

    <tr>
      <td><code>keep-all</code></td>
      <td>
        Word breaks should not be used for Chinese, Japanese, or Korean (CJK) text.
        Non-CJK text behaves the same as <code>normal</code>.
      </td>
    </tr>
  </tbody>
</table>
...
You can set a fixed height for a `<div>` element and enable vertical scrolling when the content exceeds that height.
...
Fix height of an image with vertical scrollbar:

```html
<!-- Example: fixed height div with vertical scrollbar -->
...
Q: How to write markdown inside html tags?  
A: Use `<div markdown="1"> ... </div>` to wrap the markdown content.
...
Then the content inside the `<div>` will be parsed as markdown.

Otherwise, markdown syntax inside html tags will NOT be rendered.

Only certain HTML tags support markdown parsing (`markdown="1"` attribute), including `<div>`, `<section>`, and `<article>`. 
You <span class="env-orange">cannot</span> use `<p>`, `<span>`, `<u>`, etc.

--------------------------------------------------------------------------------

Note that you can have html tags inside markdown.

```markdown
**<u>This will be bold</u>** 
```

shows as **<u>This will be bold</u>** (bold and underlined).

But 

```markdown
<u>**This won't be bold.**</u>
```

shows as <u>**This won't be bold.**</u> (only underlined, NOT bold).
