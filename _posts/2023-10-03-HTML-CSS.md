...
- Now we run into the usage of the word **property**. Each property consists of a property-key and it’s associated VALUE, separated by a colon `:`. The syntax is `property-key:value`. If you have more then one property you use the <span style='color:#008B45'>semi-colon `;` as separator. </span> E.g., 
  ...
  - `font-size: 12px;` 字号 sets font size to be 12px. It is recommened to use a relative size (e.g., 1em) than absolute size because it allows the font to scale with user browsers.
