...
Q: What is my NAS IP? \
A: "控制面板"中，找到【网络设置】，点击【网络连接】。可以查看到当前分配给 NAS 设备的 IP 地址。E.g., my IP: `192.168.10.209`. 这个 IP 地址是<span class="env-green">局域网内的地址</span>，只有在连接到同一局域网的设备上才能访问。如果需要远程访问 NAS，可以使用 [UGREENlink](#ugreenlink) 服务，无需公网 IP 或复杂的端口转发设置。
...
1. 正常关机: 长按电源键5秒以正常关机。
2. 断开电源: 关机后，拔掉电源线，等待几分钟后再重新插上。
3. 重新开机: 重新连接电源线，按下电源按钮启动设备。观察指示灯状态，确认是否恢复正常。
...
- **备用机**
  ...
  - 高级设置
    ...
    主力机上 `.gitignore` 里面的文件双向同步时，会被删除。见下文[Git 版本控制和绿联云同步冲突](#git-sync-conflict)。
...
- 选项二 (不推荐): 或者，两者并行，以 Git 版本控制为主。❌
  ...
  ‼️ 这种方式<span class="env-orange">**数据丢失**风险较大</span>，可能会导致 Git 版本控制文件夹被绿联云同步覆盖。
...
**手动同步 Git 控制的文件夹 (选项一):** 
  ...
  - 强制覆盖本地代码为远程最新代码 (简单粗暴)
    ...
    Pull from the remote repository and overwrite local changes to match the remote exactly.
...
下载东西（互联网）和与 NAS 的传输（局域网）速度理论最高只有 120MB/s 左右，实际在 70~100MB/s。如果是无线wifi 还会有损耗（距离、穿墙、设备能力等），5-60 MB/s 甚至更低也很正常。
...
Q: 如何关机?  
A: 使用绿联 UGOS Pro，点击用户头像，选择关机。或者使设备开关按钮，长按约 15s，直到听到 "滴"声后，设备将开始关机。参考 [如何将您的绿联NAS设备关机?](https://www.ugnas.com/tutorial-detail/id-13.html)

<img src="https://www.ugnas.com/file/collection/2025062317165888.jpg" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:80%;" />