...
Mac 下名字叫 「鼠须管」squirrel <img src="{{site.baseurl}}/images/rime logo.png" alt="" width="30" height="30" style="vertical-align: center;"/>
...
[emacs: 汉字上屏的问题](https://emacs-china.org/t/topic/503/14)

[输入比较快时，英文字母直接被插入到光标出，不能作为拼音组词](https://github.com/fcitx/fcitx/issues/197)
...
- 设定项速查手册、`*.schema.yaml` 详解: 
  - Rime 输入方案设计书: <https://github.com/rime/home/wiki/RimeWithSchemata>@
    
    `<shema-ID>.schema.yaml` 文件里面的设定项详解。
  - [@西山晴雪, RIME输入法方案配置手册](https://xishansnow.github.io/posts/41ac964d.html)
- [Rime Input Method Engine](https://docs.nopdan.com/rime/UserGuide.html)
...
切换输入方案: <span class="env-green">Ctrl + `` ` ``</span> 

分隔拼音: `'` (single quote) 如 `xi'an` → 西安

符号表情 以<span class="env-green">`v` 开头的拼音缩写</span>可以输入各种符号表情:
...
[`plum`](https://github.com/rime/plum) 是 Rime 输入法下的配置管理工具，提供了安装输入方案的功能。
...
- 〔用户词典〕 `<词典名>.userdb/` 或 `<词典名>.userdb.kct`
  ...
  根据用户使用习惯，输入法自动生成的 "<span class="env-green">用户数据</span>" (User Data)。包括实际上屏 (即在候选列表中选中，完成输入) 过的所有词汇及其上屏次数。
...
- 新建一个扩充词典文件，如 `luna_pinyin.extended.dict.yaml`。
  ...
  雾凇拼音的[扩展词库](https://dvel.me/posts/rime-ice/#长期维护词库)
...
- ❗<span class="env-orange">无法通过输入法自带的同步功能被同步。</span>
...
1. 新建词库文件 `my_dict.dict.yaml`，输入以下内容
   ...
   ctrl+V > Tab 键来输入 Tab 字符。

   - 词汇: 输入法候选词列表中显示的内容，可以是单字、词组、短语，甚至是整句话。
     
     可以有特殊符号，如空格、逗号、句号等。

   - 编码: 触发输入法候选词列表中显示该词汇的输入码
     
     多音节词的输入码可以用空格分隔开来，如你可以设置 `you xiang` 来输入你的名字「menghan」:

     ```
     menghan	you xiang	3
     ```

     输入 `youxiang` 就会显示「menghan」这个词了，方便你继续完整邮箱。
   
   - 权重 (词频): 可选项，数值越大优先级越高。当输入码对应多个词汇时，权重较大的词汇会排在前面。

   `dict.yaml` 文件的格式和内容可以参考 [HERE](https://rimeinn.github.io/zh-Hant/rime/dict.html).

2. 配置输入方案，挂载自己的词库。
   
   以雾凇拼音为例，在输入方案的 `rime_ice.dict.yaml` 的 <span class="env-green">`import_tables` 字段添加 `my_dict`</span>，如下所示：
...
3. 重新部署输入法，输入相应的编码就可以显示相应内容了。比如输入 `vue` 就会显示「主儿」这个词了。
   ...
   词库文件大的情况下，首次部署会比较慢，需要等一会儿。
   如果下次部署前没有修改词库，就会很快完成部署。[↩](https://dvel.me/posts/rime-ice/#其他杂项小说明)

ref:

- [雾凇拼音: 挂载自己的词库](https://dvel.me/posts/rime-ice/#挂载自己的词库)
...
1. Rime 用户文件夹下，新建 `~/Library/Rime/custom_phrase.txt`
   ...
   输入内容，每行由这些组成： `词汇`、`编码`、`权重` (决定重码的次序、可选)，它们之间使用 `Tab` 制表符分隔，其中权重可以省略。
   权重数值越大优先级越高。
   如果备选词有多个音节，用空格隔开。
...
2. 载入词库以启用自定义短语。
   ...
   这里的 `@2` 是指 `engine/translators` 列表中的第三个元素。<span class="env-green">从0开始计数。</span>
   ...
   **Attributes:**
   ...
   See [HERE](https://github.com/LEOYoon-Tsaw/Rime_collections/blob/master/Rime_description.md#三translator) for all attributes of a translator.
  
  
   <hr>
  
   **雾凇拼音**中用户自定义词典翻译器的位置是第五个位置 (`@4`)，见 `rime_ice.schema.yaml` 文件中的 `engine/translators` 列表。
   ...
   更多关于[translator 翻译器](https://ksqsf.moe/posts/2023-06-01-rime-double-pinyin/)。
...
<span class="env-green">`db_class`</span> 值默认为 `userdb`

- `userdb`: 即<span class="env-green">二进制文件</span>，输入过的内容会记录在 `*.userdb/` 文件夹中，<span class="env-green">只有在同步后</span>才能在同步目录看到人类可读的用户词典。
  
  <span class="env-green">如有多平台同步需求，使用默认值 `userdb`</span>。
...
- [雾凇拼音: 同步](https://dvel.me/posts/rime-ice/#同步)
...
- 用户配置文件目录 `/Users/<user-name>/Library/Rime/`
  ...
  See [HERE](https://github.com/my1396/Rime.git) for a GitHub backup for the Rime configuration files.
  
  ❗The GitHub repo is <span class="env-red">**private**</span> for security reasons because it contains sensitive user data. 
  User input history is highly sensitive, keep your user dictionary (`.userdb.txt`) secure and private.
...
- 注意修改配置后需要<span class="env-green">重新部署</span> (deploy) 输入法。

  如果担心之后的配置出了问题找不到，可以每一步都重新部署一次，确保代码没有出现问题。
  
  部署后出现错误，请查看 INFO 日志 ([参考](https://github.com/rime/home/wiki/RimeWithSchemata#關於調試))，找到行首字符为 E 的记录，根据错误信息以及上下文排查出错的配置文件。
...
Rime 支持自定义皮肤主题，修改 [`squirrel.custom.yaml`](https://github.com/rime/squirrel/wiki/squirrel.yaml-配置指南) 文件中的 `style` 字段，例如：
...
<table>
  <tr>
    <td></td>
    <td></td>
    <td colspan="2" style="text-align: center;"><code>inline_candidate</code></td>
  </tr>
  <tr>
    <td></td>
    <td></td>
  ...
  <tr>
    <td rowspan="2" style="text-align: center;"><code>inline_preedit</code></td>
    <th style="text-align: center;">true</th>
    <td>第一候选及未成字编码皆行内显示，候选框没有编辑区</td>
    <td>输入码行内显示，候选框没有编辑区</td>
  </tr>
  <tr>
    <th style="text-align: center;">false</th>
    <td>第一候选行内显示，候选框有编辑区</td>
    <td>行内显示空白占位符，候选框有编辑区</td>
  </tr>
</table>
...
- [squirrel.yaml 配置指南](https://github.com/rime/squirrel/wiki/squirrel.yaml-配置指南)
...
- <span class="env-green">`engine/translators`</span>: 翻译器列表，完成由内部编码到文字的翻译。
  ...
  - 注意<span class="env-green">序号从 0 开始计数</span>，因此 `@1` 是指 `engine/translators` 列表中的第二个元素。
...
A: iOS 下有个 app 叫 <img src="https://drive.google.com/thumbnail?id=1zeeRYCunm5r8ryp-gr3L4bFyLl9evEcA&sz=w1000" alt="" style="zoom:10%; border-radius:40px;" /> [仓输入法](https://apps.apple.com/cn/app/仓输入法/id6446617683?ign-itscg=30200&ign-itsct=apps_box_badge)
...
- [iPhone 拼音方案](https://www.igeekbb.com/2023/04/29/RIME/#iPhone-拼音方案)

快速入门: 方案选<span class="env-green">「仓·九宫格」</span>，键盘布局选「九宫格」。
...
<div class="note">
❗更新版本后，<code>t9.schema.yam</code> 会被覆盖。如果发现九宫格输入法输入的是数字，且没有中文候选字，可能是因为新版本中 <code>t9/isDisplayOriginalPreedit</code> 选项恢复了默认值 <code>true</code> 了。需要手动修改回为 <code>false</code>。
</div>

<span class="env-green">Workaround:</span> 新建 `t9.custom.yaml` 文件，以补丁的方式在其中添加上述配置项，这样更新就不会被覆盖了。
...
[`__include`](https://github.com/rime/home/wiki/Configuration#包含) 语法用来引入其他配置文件的内容。目标配置节点的路径以 `<filename>:/` 开始，可省略扩展名 `.yaml`。

--------------------------------------------------------------------------------

**雾凇拼音**推荐用 26 键输入法，九宫格输入法也支持，但需要额外配置。

Q: 九宫格下，雾凇拼音输入的是数字，且没有中文候选字？   
A: 需要修改方案配置文件 `rime_ice.schema.yaml`:

1. 添加 `engine/processors/t9_processor` 选项，并且 `t9_processor` 必须要在列表**首位**。
...
- [「仓输入法」: 如何启用中文九键](https://ihsiao.com/apps/hamster/docs/guides/chinese-ninekey-configuration/#如何启用中文九键)
- [rime-ice Issues: 使用九宫格布局只能输数字](https://github.com/iDvel/rime-ice/issues/731)
- [rime-ice Issues: 无法输入拼音, 按出来全是数字](https://github.com/iDvel/rime-ice/issues/975)
...
- RIME 自动同步功能只支持 <span class="env-green">`db_class: userdb`</span> 类型（二进制格式）的词库。
...
- 用户自定义词库 (`<schema-ID>.dict.yaml` 和 `custom_phrase.txt`) <span class="env-orange">不支持</span>自动同步功能，需要手动复制粘贴到其他设备的用户文件夹中。
  
  `<schema-ID>.dict.yaml` 和 `custom_phrase.txt` 是<span class="env-green">用户自定义</span>的词库文件，格式为文本，不属于 Rime 同步功能支持的 `userdb` 类型，因此无法自动同步。需要手动复制粘贴到其他设备的用户文件夹中。

更多关于用户数据和用户词库见<a href="#用户数据和用户词库">这里</a>.

--------------------------------------------------------------------------------

### 配置同步文件夹

<img src="https://drive.google.com/thumbnail?id=1fvXJ-K4Gm0ZfTTWs61sLWgAeQlG3mvje&sz=w1000" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:80%;" />
...
目前在 <img src="https://drive.google.com/thumbnail?id=1zeeRYCunm5r8ryp-gr3L4bFyLl9evEcA&sz=w1000" alt="" style="zoom:10%; border-radius:40px;" /> [「仓输入法」](https://ihsiao.com/apps/hamster/docs/guides/sync/)中，有两个不同的同步功能：「RIME 同步」和「iCloud 同步」。
...
- RIME 引擎的同步功能，主要是同步不同设备的词库文件 (当然也包含一些配置文件)。→ 设备间词库同步用 ✅
  ...
  若输入方案的 `db_class` 是 `tabledb` 或者 `stabledb`, 参考<a href="#修改dbclass">这里</a>，修改为 <span class="env-green">`userdb`</span>，即可使用 RIME 同步功能。
...
如果你的输入方案 `db_class` 为 `userdb`，Deploy (部署) > Sync user data (同步用户数据) 后，在同步文件夹 (`<sync_dir>`) 中会产生 <span class="env-green">`rime_ice.userdb.txt`</span> 格式的词库文件。
...
**iPhone <img src="https://drive.google.com/thumbnail?id=1zeeRYCunm5r8ryp-gr3L4bFyLl9evEcA&sz=w1000" alt="" style="zoom:10%; border-radius:40px;" />「仓输入法」 中 RIME 同步功能的设置步骤**
...
3. 修改「同步路径」为 `<sync_dir>`。
   ...
   `<sync_dir>` 是<span class="env-green">之前设置的</span>所有设备共享的母同步文件夹。
   ...
   <div style="display: flex; justify-content: center; gap: 10px;">
     <img src="https://drive.google.com/thumbnail?id=1t3ujSm2arwRhy0B_LnZlgK-OiueRJFix&sz=w1000" alt="" style="width: 40%;" />
     <img src="https://drive.google.com/thumbnail?id=1JWQn32jtO59nanwOBZYvc8IADWJt_jK4&sz=w1000" alt="" style="width: 40%;" />
   </div>
   ...
   当你同步用户数据时，`rime_ice.userdb.txt` 会被 RIME 同步功能自动<span class="env-green">合并同步</span>到其他设备上，确保在不同设备上使用相同的输入习惯和词库数据。

   <hr>
   
   错误示例

   上面设置的 `sync_dir` 是 iCloud 同步的路径，但是按 RIME 同步，显示如下错误: <span class="env-orange">"无写入权限"</span>

   ```
   同步失败 无写入权限: "/private/var/mobile/Library/Mobile Documents/com~apple~CloudDocs/Documents/config_files/RimeSync"
   ```
   
   <img src="https://drive.google.com/thumbnail?id=1Y3JXEZBu-KRJln3H6ojLQenvWJhft9kf&sz=w1000" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:30%;" />
...
- [@ShawLocke，如何在 Mac 和 iOS 中保持个人词典同步](https://github.com/imfuxiao/Hamster/issues/231#issuecomment-2206498995)
- [@Apus，高效同步，便捷备份：我的 Rime 输入法 Mac 与 i(Pad)OS 多设备使用方案](https://utgd.net/article/20231)
- [@Minja，Rime 词典管理及其自动化](https://utgd.net/article/20126#备份词典数据)
...
If you don't want to manually sync `custom_phrase.txt` every time, you need to use the [`userdb`](#db-class) type for your input schema, and then use RIME's built-in synchronization feature to automatically sync the user dictionary `<schema-ID>.userdb.txt` across devices. Here's how to set it up:
...
- [「仓输入法」使用指南: 词库同步](https://ihsiao.com/apps/hamster/docs/guides/sync/)
- [Dvel's Blog, 雾凇拼音: Rime 配置之多设备同步 ](https://dvel.me/posts/rime-ice/#多设备同步)
- [Hamster Github Issues: 多台移动设备可以互相同步用户日常使用而生成的词库吗？](https://github.com/imfuxiao/Hamster/issues/231)
...
[雾凇拼音: 功能演示和使用教程](https://github.com/iDvel/rime-ice#功能演示和使用教程)

**安装**

可以使用 [/plum/](https://github.com/rime/plum) ℞ 配方管理工具，一行命令就能安装和更新雾凇拼音：
...
- **Symbols** 特殊符号、字符输入: `v` + 拼音首字母缩写
  ...
  Rime 的预设配置是以 `/` 前缀开头输出一系列字符，自定义的 <span class="env-green">`symbols_v.yaml`</span> 修改成了 `v` 开头。
  ...
  <span class="env-green">`V`+全拼</span>，也可以显示 emoji 表情。具体定义见 `opencc/others.txt`，配置见 `rime_ice_dict.yaml`。
  比如 `V花` 会显示 🌸 🌺 🌼 🌻 🌷 …
...
- 以词定字
  ...
  有时候你想输入一个单字，但这个字的拼音和其他字一样，这时候可以先输入这个字所在的词语，用 `[` 和 `]` 选择开头或者结尾的字。例如，输入 `nihao`，`[` 选择 "你"，`]` 选择 "好"。
  
  快捷键写在了 `default.yaml`。[↩](https://dvel.me/posts/rime-ice/#方案设计)
...
- [`lotem/luna_pinyin.schema.yaml`](https://gist.github.com/lotem/2320943) gist.
- [oh-my-rime 输入法: 模糊拼音](https://www.mintimate.cc/zh/guide/fuzzyPinyin.html)
...
Q: 如何去除的多余的输入方案配置文件? 有许多未使用的输入方案的配置文件、替身，我并未在 `default.custom.yaml` 中开启这些方案，对我来说，他们不但没有什么用，而且还影响了快速地找到使用中的输入方案配置文件。所以怎么操作，才能去掉这些无用的配置文件？[↩](https://github.com/rime/squirrel/issues/165)  
...
比如[倉頡輸入法](https://github.com/rime/rime-cangjie)的配置文件如下
...
- **emoji 映射**
  ...
  - <span class="env-green">`emoji.txt`</span> 该文件中包含 emoji 词库。
  ...
  [OpenCC 配置](https://www.mintimate.cc/zh/guide/openccEmoji.html#opencc)
