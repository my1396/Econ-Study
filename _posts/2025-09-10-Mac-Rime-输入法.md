...
Note: 将雾凇拼音 `db_class` 设为 `userdb` 后，在同步文件夹 (`sync_dir`) 中会产生 `<schema_id>.userdb.txt` 格式的词库文件。这个文件中包含了用户的输入习惯和词库数据，会被 RIME 同步功能自动同步到其他设备上，确保在不同设备上使用相同的输入习惯和词库数据。

以下是我在**雾凇拼音**方案配置文件 `rime_ice.custom.yaml`中的相关配置：
...
patch:
  ...
  custom_phrase:
    dictionary: ""
    user_dict: custom_phrase
    # db_class: stabledb         # stabledb 是文本格式
    db_class: userdb           # userdb 是二进制格式，支持设备间同步
    enable_user_dict: true     # 启用用户词典，记录用户输入内容。
    enable_completion: false   # 补全提示
    enable_sentence: false     # 禁止造句
    initial_quality: 99        # 权重设置，custom_phrase 的权重应该比 pinyin 和 melt_eng 大
```

写入相关配置后，Deploy (部署) > Sync user data (同步用户数据)。 
若配置成功，在同步文件夹 (`<sync_dir>`) 中会产生 <span class="env-green">`rime_ice.userdb.txt`</span> 格式的词库文件。
