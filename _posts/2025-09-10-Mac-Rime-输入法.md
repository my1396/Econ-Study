...
目前在[「仓输入法」](https://ihsiao.com/apps/hamster/docs/guides/sync/)中，有两个不同的同步功能：「RIME 同步」和「iCloud 同步」。
...
以下是我在雾凇拼音方案配置文件 `rime_ice.custom.yaml`中的相关配置：
...
3. 修改「同步路径」为 `<sync_dir>`。
   ...
   ```
   rime iphone 同步失败 无写入权限: "/private/var/mobile/Library/Mobile Documents/com~apple~CloudDocs/Documents/config_files/RimeSync"
   ```
   ...
   再次按 RIME 同步，如果文件夹路径设置正确，会显示 "**RIME 同步中，请稍候...**" 的提示，之后会显示 "**同步成功**" 的提示。
...
在「仓输入法」中,「输入方案」>「文件管理」>「应用文件」会看到 `rice_ice_userdb` 文件夹，里面有二进制文件，这些文件就是用户词典文件，包含了用户的输入习惯和词库数据。当你同步用户数据时，这些文件会被 RIME 同步功能自动同步到其他设备上，确保在不同设备上使用相同的输入习惯和词库数据。
...
If you don't want to manually sync `custom_phrase.txt` every time:

- 将所有平台的 `installation.yaml` 文件的 `sync_dir` 字段设定为同一个目录，比如 iCloud、Dropbox 的目录。
- 在你输入过内容后，配置目录下会生成 `<scheme-id>.userdb/` 文件夹，里面是二进制的用户词典。
