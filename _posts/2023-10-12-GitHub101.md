...
1. Initialize an existing project to start tracking with `git`.
   ...
   - Type `git add .` to add all of the files and folder in the <span class="env-green">current diretory</span>. This step is called "<span class="env-green">staging</span>."
     
     - You’ll probably want to create a `.gitignore` file **before** you `git add`, to indicate all of the files you don’t want to track. 
     
     - If you are in a subdirectory, you can use <span class="env-green">`git add -A`</span> to add all files in the entire project.
       
       `git add --all` (two dashed for the full option) is equivalent to `git add -A` (one dash for the shorthand abbreviation option).
     
     <div class="note"> <i class="codicon codicon-lightbulb-sparkle env-green" aria-hidden="true" style="font-size:1.5em; vertical-align: middle;"></i>
     <code>git add .</code> stages all changes in the current directory and its subdirectories, while <code>git add -A</code> stages all changes in the entire repository, regardless of the current directory. <br/><br/>
     
     If you are in a subdirectory and want to stage changes from the entire project, use <code>git add -A</code>. It is more robust.
     </div>
...
2. Create a online repo and connect it to your local git project.
   ...
   <img src="{{site.baseurl}}/images/new repo.png" alt="" style="display: block; margin-right: auto; margin-left: auto; zoom:80%;" />
   </li>
   ...
   # Increase the buffer size to 500 MB to avoid issues when pushing large files
   # This is to avoid the HTTP 400 curl 22 error
   ...
   `git branch -M main` renames the current local branch to `main`. This is necessary because GitHub uses `main` as the default branch name for new repositories, while older Git repositories often used `master`. By renaming your local branch to `main`, you <span class="env-green">ensure that it matches the default branch name on GitHub</span>, which can help avoid confusion and streamline the workflow when pushing changes to the remote repository.

   To configure the initial branch name as `main` to use in all new repositories, you can run the following command:
...
**Issue:** When I `git pull`, I get the error message: "Unable too pull when changes are present on your branch." It asked me to stash changes first.

**Cause:** `git pull` refuses to silently destroy local changes. If you have local changes that conflict with the changes in the remote repository, Git will prevent you from pulling to avoid losing your work.

**Fix:** Two options:

1. You can stash your local changes, then pull the remote changes, and finally re-apply your local changes. → Safe option if you want to keep your local changes.
...
2. If you want to discard your local changes and match the remote exactly. → radical option, only if you are <span class="env-orange">absolutely sure you want to discard your local changes</span>.

   You can run: 
   
   1. Sanity check — are there unpushed commits you'd actually lose?

      ```bash
      git fetch origin             # 获取远程最新代码
      git log --oneline origin/main..HEAD     # empty = nothing to lose
      ```

      `git log --oneline origin/main..HEAD` 

   2. Discard everything and match the remote `origin/main` exactly

      ```bash
      git reset --hard origin/main # 强制覆盖本地代码为远程最新代码
      ```
   
   3. Verify your local repo status.

      Afterward, git status should be clean.
      
      `git log --oneline -1` should show `d7b39b4 add setup local cache script` (the commit short hash (commit's ID), followed by the commit message of your latest push).
      
   Note: Never run `git clean -fdx`. This will remove all untracked files and directories, including ignored files. `-fd` refers to files and dir, `-x` refers to symbolic links. ← <span class="env-orange">This is a very dangerous command.</span> You will lose files that you don't want to put in the remote repo, e.g., `_exam/` folder with confidential information. 
...
git reset --hard origin/main  # reset the current branch to match the remote branch, discarding local changes
```

Note that `git reset --hard origin/main` only resets tracked files.
Sometimes, your local repo has untracked files, e.g., sync conflict files from a cloud service or NAS. In this case, you can use `git clean -fd` to remove untracked files and directories.

To prevent future sync conflicts, consider using `.gitignore` to ignore untracked files that are not relevant to your project, such as sync conflict files.

```bash
*.sync-conflict-*
```

This will tell Git to ignore any files that match the pattern `*.sync-conflict-*`, which are typically generated by cloud services during sync conflicts.
...
**<span style='color:#00CC66'>Rename / Move</span> a file, a directory, or a symlink**
...
<span class="env-green">`git rm`</span> The primary function of `git rm` is to remove/<span class="env-green">delete</span> tracked files from the Git index. Additionally, `git rm` can be used to remove files from both the staging index and the working directory.
...
-   `-r` the option is shorthand for '<u>r</u>ecursive'. When operating in recursive mode `git rm` will remove a target directory and all the contents of that directory.
    ...
    `git rm -r folder_name` will delete the entire folder and its contents from your disk and stages the deletion.

-   <span class="env-green">`--cached` stop tracking a file in Git but keep it on your computer.</span>
    
    Note: The `--` (two dashes) separator option is used to explicitly distinguish between a list of file names and the arguments being passed to `git rm`. 
    
    `git rm --cached filename` only removes the file from the staging index, but does NOT delete the file from your disk. This is useful when you want to stop tracking a file in Git but keep it in your working directory.

    Use case scanario: `_exam/` folder contains confidential exam questions. So you put it in `.gitignore`.
    
    But if you accidentally added it to the repo previously, now you want the repo to stop tracking it, only keep it locally. 
    You run `git rm --cached _exam/` to stop tracking it in Git.

    See [HERE](#stop-tracking) for a detailed explanation of how to remove a previously tracked file from the remote repository while still keeping it locally.


Below is a summary how different commands affect tracked, untracked, and ignored files:

| Command | Tracked changes | Untracked files | Ignored files |
| ------------------ | --------- | -------- | -------- |
| <span class="env-green">`git reset --hard`</span> | discarded | **kept** | **kept** |
| `git clean -fd`    | untouched | <span class="env-orange">deleted</span> ← ❗Dangerous  | **kept** |
| `git clean -fdx`   | untouched | deleted  | **deleted** ← ‼️ the one to avoid |



<div class="note"><i class="codicon codicon-lightbulb-sparkle env-green" aria-hidden="true" style="font-size:1.5em; vertical-align: middle;"></i>
If you want to disregard unstaged changes, you can use <code>git fetch origin && git reset --hard origin/main</code>. This restores the tracked files to the state of the last commit from the remote repository, while your private untracked files remain untouched.
</div>
...
<span class="env-green">`git restore .`</span> discards all *unstaged* changes. Restore to the staged version. This is in contrast to `git reset --hard`, which discards all *staged* and *unstaged* changes, restoring to the last commit.

For one specific file use: `git restore <path/to/file/to/revert>`



`git log --oneline` Show a compact summary of commit history in one line per commit.

```bash
$ git log --oneline

3f5a8c2 Add new feature
a1b2c3d Update README.md
e4f5g6h Initial commit
```

You get the commit hash and the commit message in a concise format.

A <span class="env-green">commit hash</span> is a unique identifier, usually 7 or 8 characters, for each commit in Git. 
SHA-1 hashes are a much longer string of characters. Git uses it internally tyo identify commits.
For easy human readability, Git abbreviates the SHA-1 hash using the beginning of the full hash. This abbreviated hash is usually sufficient to uniquely identify a commit within a repository.
...
- `/foo/` will match a direcotry `foo` only at the <span class="env-green">**root**</span> of the repository and its contents. 
  
  The leading slash `/` indicates that the pattern is relative to the root of the repository.
  It will <span class="env-orange">NOT</span> match `foo/` in subdirectories.
  
  ```
  .
  ├── .gitignore
  ├── foo/               ← ignored
  └── version_1/foo/     ← NOT ignored
  ```

- `foo/` will match a **directory** `foo` <span class="env-green">**at any level**</span> in the repository and <span class="env-green">paths underneath it</span>. 
  
  **The trailing slash** `/` indicates directory only.

  It will match `foo/` at any level of the directory tree, not just the root. Such as:
...
- `foo/*` will match **all files directly** under the directory `foo`. That is, it is equivalent to have `file1.txt`, `file2.csv`, and `subfoo/` individually in `.gitignore`. Note that since `file3.txt` is in a subfolder of `subfoo/`, it will be ignored as well.
...
The difference between `foo/` and `foo/*` is significant <span class="env-green">when you want to re-include a subdirectory</span> under `foo/` later.
...
<span class="env-orange">It does NOT work</span> as `foo/` excludes everything under `foo/`.
...
With `foo/*`, `foo` itself is never excluded as a directory. 
Git will process the <span class="env-green">direct contents</span> of `foo` (`exclude_me/`, `include_me`), giving other patterns a chance to re-include <span class="env-green">subdirectories</span> like `foo/include_me/`. [↩](https://stackoverflow.com/a/5534865)
...
- `**` matches zero or more directories. For example, `foo/**/bar` matches `foo/bar`, `foo/x/bar`, `foo/x/y/bar`, and so on.

  `**/.quarto` works effectively the same as `.quarto`. They match all the following
  
  ```
  .quarto
  project/.quarto
  project/subfolder/.quarto
  ```

  Note here `.quarto` is a *file* as it does not have a trailing slash `/`. 
  If you want to ignore the `.quarto` *directory*, you can use `**/.quarto/` or `.quarto/`.
...
- `!` at the beginning of a line negates the pattern. 
  
  It re-includes a file that was previously excluded by a previous pattern.

  The order matters. The exception rule must come after the ignore rule.
  ...
  *.pdf
  ...
  !ectj template with style instructions.pdf
  ```

  - File names with spaces in the middle: do NOT need special handling. Just write the file name with spaces directly.
    
    Trailing spaces are ignored unless they are quoted with backslash (`\`). → This rarely happens.

  - You cannot re-include a file if a parent directory of that file is excluded.
    ...
    .gitignore:
    dir1/*
    !dir1/file.txt  # this won't work because dir1/ is excluded    
    ```

ref: [gitignore Documentation](https://git-scm.com/docs/gitignore)

--------------------------------------------------------------------------------

<a id="stop-tracking"></a>

#### Stop tracking a previously tracked file

How to untrack a file that was previously tracked by Git, but is now listed in `.gitignore`? I only need to remove it from the remote repository, but keep it locally.

Note that simply adding a file to `.gitignore` does not automatically remove it from the repository if it was already tracked. You need to explicitly tell Git to stop tracking the file.


`.gitignore` is only for untracked files.

```bash
# Remove "Shared folder.Rproj" from Git's index/tracking
# "Shared folder.Rproj" still reamins on your computer
git rm -rf --cached "Shared folder.Rproj"
# Stage changes (this time respecting the rules in your .gitignore).
git add .
...
*If you have several files* that you want to stop tracking, you can run
...
You can have multiple `.gitignore` files in a Git project. Git will combine then hierarchically.

- Repository-level `.gitignore` file: located at the root of the repository, it applies to all files and subfolders within the repository. 
- Folder-level `.gitignore` file: you can place a `.gitignore` file in any subfolder of the repository. It applies to files and subfolders within that specific folder and its subfolders.
  
  Rules apply relatively to that folder.

  Overrides parent rules if there's a conflict. 
...
When you make a change, but have not yet staged it (`git add`), you can undo your work.

```bash
git reset --hard <SOME-COMMIT>
```

`<SOME-COMMIT>` is some good commit that you want to go back to. You can find the commit hash by using `git log --oneline`.  
Or you can use `HEAD` to refer to the latest commit in the current branch.
...
# Move HEAD three commits back
git reset --hard HEAD~3

# Undo all local changes and match the remote repository
git reset --hard origin/HEAD
```


Undo staged (`git add`) local changes:

```bash
git reset --hard
```

Undo committed local changes. You will lose all local changes after the last commit.
...
# alternatively, to revert to the version in the last commit
...
```bash
# List all branches and show which one is currently active (with a * next to it)
$ git branch
```
...
- `git checkout <branch-name>` switch to the branch `<branch-name>`.
...
- `git branch Menghan` <span style='color:#00CC66'>create a branch</span> called `Menghan`
  ...
  // switch (checkout) to a specific branch
...
For instance, you might have the following remote branches in your repo:

- `origin/main` (the default branch, where the production-ready code lives)
- `origin/Menghan` (a branch for your development work)
- `origin/coauthor` (a branch for your co-author's development work)
...
7. Merge with the `main` branch.
   ...
   git pull origin main   # pull the latest code from remote main branch
   git merge Your-Branch  # merge your personal branch to main
   git push origin main   # push the updated main branch to remote
   ```

   Now the `main` branch contains all the changes you made in `Your-Branch` (your personal branch).
...
[All my Gists](https://gist.github.com/my1396)

Q: When should I use a Gist instead of a full-fledged Git repo?  
A: Gist is meant for sharing code snippets. Useful for sharing standalone pieces of code, such as config files, scripts, or small code examples. It shows history and allows for versioning. 

- Gist: If it's pieces of code designed to demonstrate a technique, teach a principle, or share a solution, it goes in a Gist.

- GitHub: If it's actual code intended to be run, used as is, or forked as boilerplate I put it in a proper repository. 
  
  For each project, you build a Git repository, where you can have multiple directories and organize different file types, track changes, etc.
...
A: Not good as you are putting lots of unrelated things together.
...
- Free: 50 agent mode or chat requests per mont; 2,000 completions per month
  ...
  For everyone to try out Copilot. 相当一个试用版。
...
- <span class="env-green">**Pro:**</span> **Unlimited** agent mode with GPT-4.1 and chats with GPT-4.1. Unlimited code completions.
  ...
  偶尔需要大量使用时，可能额度不够。可以以每次 \$0.04 的价格购买额外请求。
...
Error: `The error is fatal: mmap failed: Operation timed out`.  
Cause: This is caused by your git repository living on OneDrive. `.git` folder is not fully downloaded to your local machine.  
**Fix:** Right click on the `.git` folder (or the root git repository folder) and select "Always keep on this device". 
