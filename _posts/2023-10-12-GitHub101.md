...
1. Initialize an existing project to start tracking with `git`.
   ...
   - Type `git add .` to add all of the files and folder in the <span class="env-green">current diretory</span>. This step is called "staging."
...
- `foo/*` will match all files **directly** under the directory `foo`.


The difference between `foo/` and `foo/*` is significant when you want to re-include a subdirectory under `foo/` later.
...
It does not work as `foo/` excludes everything under `foo/`.
...
With `foo/*`, Git will process the <span class="env-green">direct contents</span> of `foo`, giving other patterns a chance to re-include <span class="env-green">subdirectories</span> like `foo/include_me/`. [↩](https://stackoverflow.com/a/5534865)
