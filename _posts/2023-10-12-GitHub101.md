...
2. Create a online repo and connect it to your local git project.
   ...
   3. Connect your local repo to the remote repo you just created using the following cmds. 
      ...
      Note that `main` can be replaced by `master`. 
...
- Show your remotes
  ...
  `git remote show <name>` give detailed output on the configuration of a remote `<name>`.
...
`git push origin` will push the current branch to the branch of the <span style='color:#32CD32'>matching name</span> in the remote repository (aka, “branch configured upstream”), if it exists. 
