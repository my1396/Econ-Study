...
- Proshop: 最终购入 Ugreen DXP2800 + 2 x 4TB Seagate IronWolf 够用即可
  ...
  08/10 下单; 08/14 到货; 包装完好，快递很快。满意。✅
...
[angrerett](https://www.proshop.no/Vilkaar-og-betingelser.htm?_gl=1*r9iw2e*_up*MQ..*_gs*MQ..&gclid=Cj0KCQjwnIDUBhDrARIsAJDGwStMOiR7Y1S5LZV-yHV3phVsqa5-TcoYJRBzIxNLdNCrVc1Q8a6qBHYaAhDAEALw_wcB&gbraid=0AAAAADuUrFG5IXEQVPJ7kwtqg_vOqs0Wm) 14 天无理由退货

如果你是**个人消费者（forbruker）**购买：

- 收到显示器当天开始计算 14 天
  - 14 天内通知 Proshop 你要退货
  - 通知后还有 14 天把商品寄回
  - 所以你可以收到商品后测试，再决定是否保留

- 退货运费你自己承担

  Proshop 提供的预付退货标签是：109 NOK 只适用于 20 kg 以下的包裹。

- 原包装非常重要
  
  Varen skal sendes i retur i den originale emballasjen.

- 退货前最好先联系/登记 Proshop
  
  Ved retur ... må forsendelsen avtales med oss på forhånd.

- 退款什么时候回来？

  Proshop 收到你的退货后，最迟 14 天内退款。

- 没有 angrerett 的特殊商品:

  定制电脑，打开封条的软件，数字内容，卫生用品 (直接接触人体、出于卫生原因不适合在被拆封/使用后再次销售的商品，如 ear pods)，企业客户购买
...
Creates pool and volume → Choose RAID type → Agree to do Drive Check → Schedule tests → Create Shared Folder → Set up Snapshots → Security Settings → Task Scheduler to set up a schedule for emptying your recycle bin.
...
- File system: Use <span class="env-green">**BTRFS** format</span>
...
1 GbE (Gigabit Ethernet)， 2.5 GbE 

链路聚合

理论带宽可达 5 Gbps (5 Gb, Gigabits 千兆比特)  ≈ 500 MB/s (每秒传输 500 MB, MegaBytes 兆字节)

1 MB (mega**byte**) = 8 Mb (maga**bit**)

1 Gbps (Gb/s) = 125 MB/s

Usually Internet connection speeds are measured by **Mbps** -> Megabits per Second.

That means if you have 1 Mbps connection then you will need 8 seconds to download a 1 MB file. Likewise if you have 8 Mbps connection , 1 MB file will be downloaded in one second.
