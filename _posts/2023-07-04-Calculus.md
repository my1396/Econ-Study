...
Multivariate normal random vecctors (MV-N). Let $X$ be a $K\times 1$ continuous random vector, with mean $\mu$ and covariance $\Sigma$, denoted $X\sim N(\mu, \Sigma)$. Its joint probability density function is 
