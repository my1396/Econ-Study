...
Then the Jacobian matrix of f is defined to be an m×n matrix, denoted by J, whose $(i,j)$ th entry is $\mathbf{J} _{ij}={\frac{\partial f_{i}}{\partial x_{j}}}$. \
...
函数F对于X的偏导数矩阵，就被称为雅可比矩阵(Jacobian Matrix)。
