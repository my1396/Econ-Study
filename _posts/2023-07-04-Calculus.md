...
Jacobian matrix is a matrix of partial derivatives. The Jacobian matrix of $f$ is defined to be an $m \times n$ matrix, denoted by $\mathbf{J}$, whose $(i,j)$ th entry is $\mathbf{J}_{ij}=\frac{\partial f_{i}}{\partial x_{j}}$. 
...
此矩阵表示为：$J_F(x_1,\cdots ,x_2)$，或者: $\frac{\partial (y_1,\ldots,y_n)}{\partial (x_1, \ldots ,x_n)}$ or $\frac{\partial (f_1,\ldots,f_n)}{\partial (x_1, \ldots ,x_n)}$
